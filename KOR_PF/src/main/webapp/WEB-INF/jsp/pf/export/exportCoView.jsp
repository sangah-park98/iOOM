...
<head>
  ...
  <title>FTA 원산지증명서 현황</title>
  <script src="/pf/js/function.js"></script>
  <script src="/pf/js/export/exportCoView.js?v=<%=fmt.format(today)%>" charset="UTF-8"></script>
...
<main class="p-2 grow flex flex-col gap-2">
  <form class="w-full shrink-0 grid grid-rows-3 grid-cols-[auto_repeat(5,1fr)_auto_repeat(5,1fr)]
    ...
    <select
      ...
      <option value="01" selected>요청일자</option>
      <option value="02">발행일자</option>
    </select>
    <!-- Calendar -->
    <div
      id="exportCoView_div1"
      ...
      <div class="relative grow">
        ...
        <input type="text" class="form-control band-calendar text-base bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full ps-10 py-1 px-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
				id="exportCoView_srch2" name="exportCoView1_date"
      ...
      <div class="relative grow">
        ...
        <input type="text" class="form-control band-calendar text-base bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full ps-10 py-1 px-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
				id="exportCoView_srch3" name="exportCoView2_date"
    ...
    <div id="exportCoView_div1" class="col-start-7 row-start-2 col-span-4 flex items-center gap-1">
      <button
          type="button"
          id="today-button"
          onclick="fn_chgDate1()"
    ...
    <input type="text" id="exportCoView_srch9"
  ...
  <div class="flex items-center justify-between w-full shrink-0">
    ...
    <div class="flex items-center gap-2">
      <label for="default-radio-7" class="flex items-center gap-1">
        <input id="default-radio-7" type="radio" name="exportCoViewType" value="read" checked
          class="w-4 h-4 text-primary-600 bg-gray-100 border-gray-300 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
        <span>읽기</span>
      </label>
<!--       <label for="default-radio-8" class="flex items-center gap-1">
        <input id="default-radio-8" type="radio" name="exportCoViewType" value="enrol"
          class="w-4 h-4 text-primary-600 bg-gray-100 border-gray-300 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
        <span>등록</span>
      </label>
 -->      <label for="default-radio-9" class="flex items-center gap-1">
        <input id="default-radio-9" type="radio" name="exportCoViewType" value="edit"
          class="w-4 h-4 text-primary-600 bg-gray-100 border-gray-300 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
        <span>편집</span>
      </label>
    </div>
    ...
    <div class="flex items-center gap-2">
       <button
        type="button"
        id="btnExportCoSave" onclick="fn_saveItemViewMemoCheck()"
  ...
  <!-- MainTable -->
  <div id="exportCoViewTable" class="grow bg-white shadow-sm h-full max-h-[100rem] rounded-lg border border-slate-200 z-0"></div>
  <!-- MainTable -->
</main>
...
<div id="docuFileListPopUp" class="modal fade fixed top-0 left-0 h-full w-full z-[200] bg-black/50 items-center justify-center duration-300" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" style="display: none; ">
	<div class="modal-close absolute top-0 left-0 w-full h-full"></div>
		<div class="modal-dialog modal-xl" style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); width:600px;" >
			<div class="modal-content bg-white shadow-lg relative rounded min-w-80 overflow-hidden">
				<div class="pl-7 pr-5 py-2 text-white bg-primary-900 flex items-center justify-between">
					...
					<button type="button" onclick="filecoDocuListClose()" class="modal-close text-2xl px-1.5 py-1 rounded-lg hover:bg-rose-500/70 border-2 border-transparent hover:border-white duration-300 flex items-center justify-center"><i class="far fa-xmark"></i></button>
				</div>
				<div class="pl-7 pr-5 py-2 flex items-center justify-end">
					<button type="button" onclick="fn_coDocuPopupDownload()" class="px-3 py-2 rounded-lg text-white hover:opacity-50 border-2 border-transparent duration-150 bg-primary-700 ">
				        <i class="fa-regular fa-download"></i>
				        	다운로드
				    </button>&nbsp;
					<button type="button" onclick="fn_coDocuPopupDelete()" class="px-3 py-2 rounded-lg text-white hover:opacity-50 border-2 border-transparent duration-150 bg-primary-700 ">
			    ...
			    <div class="mx-5 py-4 border-t-2 flex justify-end gap-2 border-t" style="z-index: 1;">
			      ...
			      <button type="button" id="uploadDocuSaveBtn" onclick="fn_fileDocuSave()" class="so-upload-button px-3 py-2 rounded-lg text-white hover:opacity-50 duration-150 bg-primary-700">
			        <i class="fa-regular fa-floppy-disk"></i>
			        	저장
			      </button>
			    </div> 
