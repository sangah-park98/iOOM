	          ...
	          <small style="color:red;">엑셀파일이 아닌 데이터는 파일업로드가 안됩니다.</small>
		         <div class="col-lg-12 rowflex">
			         ...
			         <i class="fa-regular fa-upload"></i>
	      			...
	      			<label class="cusExp_div7 col-start-1 row-start-3 col-span-1 row-span-1 flex items-center font-medium text-gray-900 pr-2">
						    일반구분
						</label>
						<div class="cusExp_div7 flex flex-row flex-wrap gap-4 col-start-2 col-span-5 row-start-3 items-center *:flex *:flex-row *:items-center *:gap-2 font-medium text-gray-900">
						    <label>
						        <input id="default-radio-1" type="radio" value="01" name="cusExpView_srch6" 
						               class="w-4 h-4 text-primary-600 bg-gray-100 border-gray-300 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
						        <span>레디</span>
						    </label>
						    <label>
						        <input id="default-radio-2" type="radio" value="02" name="cusExpView_srch6"
						               class="w-4 h-4 text-primary-600 bg-gray-100 border-gray-300 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
						        <span>엔컴</span>
						    </label>
						</div>
					 ...
					 <div class="mx-5 py-4 border-t-2 flex gap-2 border-t" style="z-index: 2;">	
	      			<label class="cusExp_div7 col-start-1 row-start-3 col-span-1 row-span-1 flex items-center font-medium text-gray-900 pr-2">
						...
						<div class="cusExp_div7 flex flex-row flex-wrap gap-4 col-start-2 col-span-5 row-start-3 items-center *:flex *:flex-row *:items-center *:gap-2 font-medium text-gray-900">
						    <label>
						        <input id="default-radio-1" type="radio" value="03" name="cusExpView_srch6" 
						               class="w-4 h-4 text-primary-600 bg-gray-100 border-gray-300 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
						        <span>DHL</span>
						    </label>
						</div>
...
<div class="modal-popup-cusExpUpl fixed top-0 left-0 h-full w-full z-[200] bg-black/50 items-center justify-center duration-300 hidden">
  <div class="modal-close absolute top-0 left-0 w-full h-full"></div>
  <!-- Uploader Modal content -->
  <div class="modal-cusExpUpl hidden modal-content bg-white shadow-xl relative rounded min-w-96 overflow-hidden">
    <div class="pl-3 pr-3 py-2 text-white bg-primary-900 flex items-center justify-between">
      ...
      <button type="button" class="modal-close-cusExpUpl text-2xl px-1.5 py-1 rounded-lg hover:bg-rose-500/70 border-2 border-transparent hover:border-white duration-300 flex items-center justify-center"><i class="far fa-xmark"></i></button>
    </div>
...
<script>
    ...
    var dropzone = new Dropzone("#dropzone-file", {
        ...
        params: function() {
           ...
           var type = $("input:radio[name=cusExpView_srch6]:checked").val();
        	//  선택되지 않았을 경우 기본값을 0으로 설정
           if (typeof type === 'undefined') {
        	   alert("타입을 선택해주세요.");
               return;  // type이 없을 때 업로드를 막음
           }
       	 console.log("type 값: " + type); // 디버깅을 위해 값 확인
           return {
               type: type
           };
       },
        init: function() {
        	...
        	document.querySelector('.modal-close-cusExpUpl').style.display = 'none';
       ...
       queuecomplete: function() {
    	   document.querySelector('.modal-close-cusExpUpl').style.display = 'block';
       }
    });
 ...
 function fn_resultCusExpUpload() {
    	
    	//업로드 Modal Action
    	const modalCusExpUpload = document.querySelector('.modal-popup-cusExpUpl')
    	const modalCusExp = document.querySelector('.modal-cusExpUpl')
    	modalCusExpUpload.classList.remove('hidden')
    	modalCusExpUpload.classList.add('flex')
    	modalCusExp.classList.remove('hidden')
    	modalCusExp.classList.add('block')


    }


    function fn_CloseCusExpUpload() {
    	const modalCusExpUpload = document.querySelector('.modal-popup-cusExpUpl')
    	//const contactModal = document.querySelector('.contact-modal')
    	const modalCusExp = document.querySelector('.modal-cusExpUpl')
    	modalCusExpUpload.classList.remove('flex')
    	modalCusExpUpload.classList.add('hidden')
    	modalCusExp.classList.remove('block')
    	modalCusExp.classList.add('hidden')
    	dropzone.removeAllFiles(true);
    	popupState = false;
    }

    const modalCloseCusExpUpload = document.querySelector('.modal-close-cusExpUpl');
    modalCloseCusExpUpload.addEventListener('click', fn_CloseCusExpUpload);
