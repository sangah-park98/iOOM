...
function toggleButton1(count) {
    ...
    if (!Editing) {
        // 처음 버튼 클릭 시
        button.textContent = '사업자 번호 인증'; // 버튼 텍스트 변경
        //compname.removeAttribute('readonly'); // 이메일 입력란 readonly 해제
        compregno.removeAttribute('readonly'); // 인증번호 입력란 readonly 해제
    } else {
        // 두 번째 버튼 클릭 시
    	regNoValidation(count); // 사업자 번호 인증 호출
    }

    Editing = !Editing; // 상태 토글
}
