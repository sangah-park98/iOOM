...
function fn_searchImportBl(){
	...
	$.ajax({
        ...
        success : function(data) {
            	...
            	$("#importBlCnt").text(data.totCnt.toLocaleString()); //검색결과 총 갯수
            	fn_loading(false);
        },
        error : function(e, textStatus, errorThrown) {
        	if(e.status == 400){
...
function fn_impBlTableCol(){
	...
	this.impBlCol = [
		...
		{data : 'type', className : "htCenter", wordWrap: false, className : "htCenter", readOnly:true, renderer : impRequestRenderer},
...
function fn_impBlTableHeader(){
	...
	this.impBlHeader = [
		"B/L번호","상태",  "신고번호", "신고일자", "수리일자", "PDF", "구분", ""
	 ] ;
}
...
function fn_blFileList(row, col){
    ...
    console.log("data.blno: " + data.blno); // O
...
function fn_fileBlSave(){
    ...
    if (fileBlEntries.length !== 1) {
        ...
        if (!(fileBlName.startsWith('BL_') || fileBlName.startsWith('bl_')) && $("#blNo").val() == "") {
...
function fn_docuPrintBl() {
    ...
    $.ajax({
        ...
        data: JSON.stringify(downloadFile), // 서버로 전송할 데이터
...
function fn_impPopupDelete () {
    var sData = {};
    var rowData = blListHot.getSourceData();
    console.log(rowData);

    if (allCheck == true) {
        if (confirm("등록된 모든 파일을 삭제하시겠습니까?")) {
            for (var i = 0; i < rowData.length; i++) {
                sData["srch1"] = rowData[i].blno;
            }

            $.ajax({
                type: "POST",
                url: "/import/impPopupDeleteList.do",
                data: sData,
                beforeSend: function(xmlHttpRequest) {
                    xmlHttpRequest.setRequestHeader("AJAX", "true");
                },
                dataType: "json",
                success: function(data) {
                	blListHot.alter('remove_row', 0, blListHot.countRows());
            ...
            $.ajax({
                ...
                success: function(data) {
                    for (let i = itemsToDelete.length - 1; i >= 0; i--) {
                    	blListHot.alter('remove_row', itemsToDelete[i]);
                    }
                    alert('파일이 삭제되었습니다.');
                },
                error: function(xhr, textStatus, errorThrown) {
