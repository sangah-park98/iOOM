...
$( document ).ready(function() {
	...
	$("#document_div6 p").html(
		'<i class="fa-duotone fa-feather text-primary-900"></i>&nbsp;&nbsp;' +
		 impCnt2 + ': <span id="documentViewCnt">0</span>'
	);
...
$("input[name=documentView_srch1]").change(function(){
	  ...
	  if (radioValue === '03') {
		  document.getElementById('blNoLabel').innerText = invoiceNo; /*Invoice 번호*/
		  document.getElementById('documentView_srch7').placeholder = msgInputInvoiceNo; /*'Invoice 번호를 입력해주세요.';*/
		  document.getElementById('poNoLabel').innerText = soNumber; /*'SO 번호';*/
		  document.getElementById('documentView_srch9').placeholder = msgInputSo; /*'SO 번호를 입력해주세요.';*/
		  $("#document_div6 p").html(
			'<i class="fa-duotone fa-feather text-primary-900"></i>&nbsp;&nbsp;' +
			expCnt2 + ': <span id="documentViewCnt">0</span>'
		  );
        
	  } else {
		  document.getElementById('blNoLabel').innerText = 'B/L 번호';
		  document.getElementById('documentView_srch7').placeholder = msgInputBlNo; /*B/L 번호를 입력해주세요.*/
		  document.getElementById('poNoLabel').innerText = poNumber; /*'PO 번호'*/
		  document.getElementById('documentView_srch9').placeholder = msgInputPo; /*'PO 번호를 입력해주세요.';*/
		  $("#document_div6 p").html(
			'<i class="fa-duotone fa-feather text-primary-900"></i>&nbsp;' +
			impCnt2 + ': <span id="documentViewCnt">0</span>'
		  );
...
function fn_documentViewTableCol(){
	...
	if(searchType == '02'){
		...
		this.documentViewCol = [
			{data : 'checkBox', type : 'text', className : "htCenter", width: 60,type: 'checkbox', checkedTemplate: 'yes', uncheckedTemplate: 'no', readOnly:false},
			{data : 'expFirm', className : "htCenter", width: 150, wordWrap: false, className : "htCenter", readOnly:true},
			...
			{data : 'cb', className : "htCenter", width: 150, className : "htCenter", readOnly:true},
			{data : 'uc', className : "htCenter", width: lang === 'en' ? 170 : 150, className : "htCenter", readOnly:true},
			{data : 'ci', className : "htCenter", width: 150, className : "htCenter", readOnly:true},
			{data : 'pl', className : "htCenter", width: 150, className : "htCenter", readOnly:true},
			{data : 'bl', className : "htCenter", width: 150, className : "htCenter", readOnly:true},
			{data : 'co', className : "htCenter", width: 150, className : "htCenter", readOnly:true},
...
function fn_documentViewTableHeader(){
	...
	if(searchType == '02') {
		...
		this.documentViewHeader = [
			"", shipper,
			invoiceNo, rptNo, rptday, lisDay, declarationCert, docuTotal, docuCorrectionTotal,
			invoice, pl, bl, co, docuRequirement, docuEtc, docuLiquidation, factoryCd, so
			/*"", "수출자",
			"Invoice 번호", "신고번호", "신고일자", "수리일자", "신고필증", "통합", "정정 통합", 
			"Invoice", "Packing List", "B/L", "원산지증명서", "요건 서류", "기타", "정산서", "부서코드", "SO" */
		 ];
	}
}
...
function fn_handsonGridDownloadPopupOption() {
	docuDownloadPopupSettings = {
        ...
        rowHeaders: true,
        columnHeaderHeight: 25,
        colHeaders: [rptNo, declarationCert, docuTotal, docuCorrectionTotal, invoice, pl, bl, co, docuRequirement, docuEtc, docuLiquidation, ""],
...
function fn_docuGlassesBtn(row, col){
	...
	if (searchTp == '02'){
		text = blNo + ": " + data.blno + " / " + rptNo + ": " + data.rptNo.replace(/-/g, '');
	} else {
		text = invoiceNo + ": " + data.mgCode + " / " + rptNo + ": " + data.rptNo.replace(/-/g, '');
	}
...
function fn_docuListPopupHeader(){
	var searchTp = $("input:radio[name=documentView_srch1]:checked").val();
	this.docuListPopupHeader = (searchTp == '02') ? 
	  ["", blNo, fileType, "", fileName, uploadDt, ""]
	: ["", invoiceNo, fileType, "", fileName, uploadDt, ""];
			  
	/*this.docuListPopupHeader = (searchTp == '02') ? 
	  ["", "B/L 번호", "파일 타입", "",'파일명','업로드 일자', '']
	: ["", "Invoice 번호", "파일 타입", "",'파일명','업로드 일자', ''];*/
};
...
function fn_handsonGridDocuListPopupOption(col, header) {
	docuListPopupSettings = {
        ...
        mergeCells: false,
        allowInsertRow: false,
        hiddenColumns: { copyPasteEnabled: false, indicators: false, columns: [1,3,6]},
        afterGetColHeader: function(col, TH){
        	if(col == 0){
...
function fn_fileDocuModalSave() {
	...
	if(docuLoadedFiles.length == 0){
		alert(msgNoFileSave) /*"저장할 파일이 없습니다."*/
		return;
	}
...
function fn_docuPopupDownload() {
    ...
    if (cnt == 1) { // 단일 파일 다운로드
        $("#docuPath").val(docuFiles[0].docuPath);
        $("#docuFile").val(docuFiles[0].docuFile);
        $("#docuOrgFile").val(docuFiles[0].docuOrgFile);
        console.log($("#docuPath").val());
        console.log($("#docuFile").val());
        console.log($("#docuOrgFile").val());
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (searchTp =="02") {
                	...
                	zipName = docuFiles[0].invoiceNo + "_" + docuFiles[0].rptNo;
                }
                $("#zipDownloadName").val(zipName);
                document.zipDownloadForm.action = "/docu/downloadDocuFile.do";
                document.zipDownloadForm.submit();
...
function fn_docuPopupDelete() {
    ...
    if (confirm(msgDelSelectedFile)) { /*파일을 삭제하시겠습니까?*/
        $.ajax({
        	type: "POST",
