...
var kpiLeadtimeViewCol = [
	...
	{data : 'tgPlnMark', className : "htCenter", width: 150, wordWrap: false, readOnly:true},
...
function fn_purchaseAllClick(obj, table) {
	checkBox = !checkBox;
	console.log("checkBox"+checkBox, table);
	if (checkBox) {
		check = "true" ;
	} else {
		check = "false" ;
...
//	    	var totTaxSum = 0;
//	    	
//	    	for(var i=0; i < meargeJson.length; i++) {
//	    		prevTaxSum += fn_removeComma(meargeJson[i]["prevTaxSum"]);
//	    		totTaxSum += fn_removeComma(meargeJson[i]["totTaxSum"]);
//	    	};
//	    	
//	    	kpiImportViewHead[0][8]["label"] = fn_setComma(prevTaxSum);
//	    	kpiImportViewHead[0][9]["label"] = fn_setComma(totTaxSum);
//	    	
//	    	kpiImportTable.updateSettings({
//	    		nestedHeaders : kpiImportViewHead
//	    	});
