...
function fn_clearRpt() {
	...
	//기간
	var Radiobutton = document.querySelectorAll('input[name="rpt_srch1"]');
	for(var i = 0;i <Radiobutton.length; i++){
		Radiobutton[i].checked = (Radiobutton[i].value==="03");
	}
	//현재날짜
	var rptDate4 = document.querySelector('input[name="rptDate4"]');
	var today = new Date().toISOString().substring(0, 10).replace(/-/g, '/');
	rptDate4.value=today;
	// 현재날자 -3개월+1일
	var rptDate3 = document.querySelector('input[name="rptDate3"]');
	var currentDate = new Date();
	currentDate.setMonth(currentDate.getMonth() - 3);
    currentDate.setDate(currentDate.getDate() + 1);
    var formattedDate = currentDate.toISOString().substring(0, 10).replace(/-/g, '/');
    rptDate3.value = formattedDate;
...
function fn_reportExcelSrch(type){
	...
	if(type == '01'){
		...
		$("#reportExCol").val(String([plntCd,exportNation,colFtaCd,colHsCd,mfnPrice,ftaPrice2,applyYear+","]));						
		$("#reportExCd").val("plntCd,exportNation,ftaCd,hsCd,mfnPrice,ftaPrice,applyYear,");
		$("#reportExType").val("text,text,text,text,text,floatString,floatString,int,");
	}else if(type == '02'){
		$("#reportExTit").val(orgnreport);
		$("#reportExCol").val(String([plntCd,exportNation,colFtaCd,colHsCd,mfnPrice,ftaPrice2,applyYear+","]));
		$("#reportExCd").val("plntCd,exportNation,ftaCd,hsCd,mfnPrice,ftaPrice,applyYear,");
...
function fn_searchorgnResult(selectedOption){
	...
	$.ajax({
        ...
        success : function(data) {
        	...
        	$("#orgnResultCnt").text(data.totCnt); //검색결과 총 갯수
        	fn_loading(false);
        },
...
function fn_orgnTableHeader(){
	...
	this.orgnHeader = (tableType == "edit") ? [
		plntCd,exportNation,colFtaCd,colHsCd,mfnPrice,ftaPrice2,applyYear
	] : [
		plntCd,exportNation,colFtaCd,colHsCd,mfnPrice,ftaPrice2, applyYear
	];
...
function fn_orgnResult2(){
	  ...
	  resultOrgnPopupSettings = {
  columns : [
		{data : 'plntCd', type : 'text', className : "htCenter",width:80,readOnly: true},
		{data : 'exportNation', type : 'text', className : "htCenter",width:100,readOnly: true},
		...
		{data : 'hsCd', type : 'text', className : "htCenter",readOnly: true,renderer:function(instance, td, row, col, prop, value, cellProperties) {
	      ...
	      {data : 'mfnPrice', type : 'text', className : "htCenter",renderer: percentRenderer,readOnly: true},
	      {data : 'ftaPrice', type : 'text', className : "htCenter",renderer: percentRenderer,readOnly: true,width:150},
	      {data : 'applyYear', type : 'text', className: "htCenter" ,readOnly: true},
	      {
	    	  data: 'calPrice',
	    	  type: 'numeric',
	    	  className: "htRight",readOnly: true,width:230,
	    	  renderer: function (instance, td, row, col, prop, value, cellProperties) {
	    	    if (value !== null) {
	    	      const formattedValue = parseFloat(value).toLocaleString(undefined, {
	    	        minimumFractionDigits: 2,
