...
function fn_chgDate2() {
     ...
     $("#calculate_srch2").val(oneYearAgo);
     $("#calculate_srch3").val(today);
}
function fn_chgDate3() {
    ...
    $("#calculate_srch2").val(threeYearAgo);
    $("#calculate_srch3").val(today);
}
function fn_chgDate4() {
    ...
    $("#calculate_srch2").val(fiveYearAgo);
    $("#calculate_srch3").val(today);
}
...
function fn_setCalculView(){
	...
	sData["srch7"] = $("input[name=calculate1_date]").val(); // srch7: 신고일자 처음
	sData["srch8"] = $("input[name=calculate2_date]").val(); // srch8: 신고일자 끝
...
function fn_calculateTableCol(){
		...
		this.calculateCol = [
			...
			{data : 'saleSupEk', className : "htCenter", width: 150, className : "htCenter", readOnly:true},
			{data : 'saleTaxEk', className : "htCenter", width: 150, className : "htCenter", readOnly:true},
			{data : 'saleSumEk', className : "htCenter", width: 150, className : "htCenter", readOnly:true},
		];
}
...
function fn_calculateDetailTableCol(){
	...
	this.calculateDetailCol = [
		{data : 'checkBox', type : 'text', className : "htCenter", width: 60,type: 'checkbox', checkedTemplate: 'yes', uncheckedTemplate: 'no', readOnly:false},
		{data : 'adjNm', className : "htCenter", width: 150, className : "htCenter", readOnly:true},
		{data : 'adjCd', className : "htCenter", width: 150, className : "htCenter", readOnly:true},
		{data : 'supEk', className : "htCenter", width: 150, className : "htCenter", readOnly:true},
		{data : 'taxEk', className : "htCenter", width: 150, className : "htCenter", readOnly:true},
		{data : 'sumEk', className : "htCenter", width: 150, className : "htCenter", readOnly:true},
...
function fn_calculateTableHeader(){
		...
		this.calculateHeader = [
			"",
			 "비용발행", "구분" ,"P.O No.", "B/L No.", "수리일자", "신고번호", "공급가액", "VAT", "합계"
		 ]; 
}
        ...
        reader.onload = function(event) {
         ...
         // 헤더 행이 비어 있을 때 다음 행을 헤더로 처리
            ...
            excelUploadHot.addHook('beforeRemoveRow', function(index, amount, physicalColumns, source) {
            	...
            	headerRow.forEach(function(header, colIndex) {
                    if (colIndex > 0 && colIndex < headerRow.length) { // 첫 번째 열은 헤더가 아님
                        ...
                        if (typeof calCodeInfo !== 'undefined') {
                            optionValues.forEach(function(optionValue) {
                                // 옵션 값이 현재 calCodeInfo의 calCode와 일치하는지 확인하여 선택 여부를 지정
                                var selected = optionValue.value === calCodeInfo.calCode ? ' selected' : '';
                                selectTag += '<option value="' + optionValue.value + '"' + selected + '>' + optionValue.text + '</option>';
            ...
            excelUploadHot.addHook('afterRemoveCol', function(index, amount, physicalColumns, source) {
            	selectedList = {};
                // 열이 삭제된 후에 Handsontable을 다시 랜더링하는 함수 실행
            	var newIndices = [];
                for (var i = 0; i < calCode2Array.length; i++) {
                    if (calCode2Array[i].Index >= index) {
                        // 삭제된 열 이후의 열의 인덱스를 조정
                        newIndices.push(calCode2Array[i].Index - amount);
                    } else {
                        // 삭제된 열 이전의 열은 변경되지 않음
                        newIndices.push(calCode2Array[i].Index);
                    }
                }

                // 새로운 인덱스 값을 기반으로 calCode2Array의 Index 재할당
                for (var i = 0; i < calCode2Array.length; i++) {
                    calCode2Array[i].Index = newIndices[i];
                }
                ...
                var lastHeader = headerRow[headerRow.length - 1];
                if (lastHeader) {
                    headerRow.push(lastHeader);
                }
...
document.querySelector('.excel-save-button').addEventListener('click', function() {
	...
	*/
	...
	for (var i = 0; i < data.length; i++) {
	    ...
	    Object.keys(selectedList).forEach(function (key, index) {
	...
	/*dbArray.forEach(dbObject => {
	    // 새롭게 정의할 객체
	    let newDbObject = {};
	    let newCalDataIndex = 1;
	    let newCalDataDetailIndex = 1;

	    // B/L No.와 Description 값을 먼저 설정합니다
	    Object.keys(dbObject).forEach(key => {
	        if (key.startsWith('calData') && !key.startsWith('calDataDetail')) {
	            let index = key.slice('calData'.length); // calData의 인덱스를 추출합니다
	            let detailKey = 'calDataDetail' + index; // 대응하는 calDataDetail 키를 생성합니다

	            if (dbObject[key] === 'B/L No.') {
	                newDbObject['blNo'] = dbObject[detailKey]; // B/L No.의 값을 설정합니다
	            } else if (dbObject[key] === 'Description') {
	                newDbObject['description'] = dbObject[detailKey]; // Description의 값을 설정합니다
	            } else {
	                // B/L No.나 Description이 아닌 경우 새 객체에 순서대로 추가합니다
	                newDbObject['calData' + newCalDataIndex] = dbObject[key];
	                newDbObject['calDataDetail' + newCalDataDetailIndex] = dbObject[detailKey];
	                newCalDataIndex++;
	                newCalDataDetailIndex++;
	            }
	        }
	    });
	...
	dbArray.forEach(dbObject => {
	    ...
	    Object.keys(dbObject).forEach(key => {
	        if (key.startsWith('calData') && !key.startsWith('calDataDetail')) {
	            ...
	            if (dbObject[key] === 'B/L No.') {
	                ...
	                newDbObject['vat'] = dbObject[detailKey]; // Description의 값을 설정합니다
	            }
	        }
	    });

	    // 나머지 값을 새 객체에 순서대로 추가합니다
	    let calDataIndex = 1;
	    let calCodeIndex = 1;
	    ...
	    Object.keys(dbObject).forEach(key => {
	        if (key.startsWith('calData') && !key.startsWith('calDataDetail')) {
	            ...
	            if (dbObject[key] !== 'B/L No.' && dbObject[key] !== 'Description' && dbObject[key] !== 'VAT') {
	                if (dbObject[key].startsWith('CMMN')) {
	    ...
	    newDbObject['partnType'] = partnType;
	    newDbObject['partnCmpnyNm'] = partnCmpnyNm;
...
function fn_searchDetailView(rptNo, type, hawb){
