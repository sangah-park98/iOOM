...
//테이블 타입 변경
$("input[name=calculateViewType]").change(function(){
...
function fn_searchCalViewInfo(){
	...
	$.ajax({
        ...
        success : function(data) {
        	...
        	// `supAmt`, `taxAmt`, `totAmt` 합계를 계산
            const sumSupAmt = data.resultList.reduce((total, item) => total + (parseFloat(item.supAmt) || 0), 0);
            const sumTaxAmt = data.resultList.reduce((total, item) => total + (parseFloat(item.taxAmt) || 0), 0);
            const sumTotAmt = data.resultList.reduce((total, item) => total + (parseFloat(item.totAmt) || 0), 0);
        	
        	// 합계 행 설정
        	var sumRow = {
                    blNo: "합계", // Summary label
                    rptNo: blNoCount + "건", // Unique B/L No. count
                    rptDay: "",
                    lisDay: "",
                    partnCmpny: "",
                    description: "",
                    supAmt: sumSupAmt.toLocaleString(),  // 합계 결과를 표시
                    taxAmt: sumTaxAmt.toLocaleString(),  // 합계 결과를 표시
                    totAmt: sumTotAmt.toLocaleString(),  // 합계 결과를 표시
                    remarks: "",
                    paymentYn: ""
                };
            ...
            calculateHot.setCellMeta(lastRowIndex, 0, 'className', 'htCenter boldText'); // blNo (0번 열) bold 처리
            calculateHot.setCellMeta(lastRowIndex, 1, 'className', 'htCenter boldText'); // rptNo (1번 열) bold 처리
            calculateHot.setCellMeta(lastRowIndex, 6, 'className', 'boldText htRight');  // supAmt 열
            calculateHot.setCellMeta(lastRowIndex, 7, 'className', 'boldText htRight');  // taxAmt 열
            calculateHot.setCellMeta(lastRowIndex, 8, 'className', 'boldText htRight');  // totAmt 열
        	...
        	/*console.log("data"+data.resultList.length);
        	console.log("totCnt"+totCnt);*/
        	$("#calculateCnt").text(totCnt-1); //검색결과 총 갯수
...
function fn_searchCalDetailView(partnCmpny){
	...
	$.ajax({
		...
		success : function(data) {
        	...
        	// 합계 행 설정
        	var sumRow2 = {
        			partnCmpny: "합계", // Summary label
        			blNo: blNoCount2 + "건", // Unique B/L No. count
        			rptNo : "",
        			supAmt: sumSupAmt.toLocaleString(),  
                    taxAmt: sumTaxAmt.toLocaleString(), 
                    totAmt: sumTotAmt.toLocaleString(), 
                    paymentYn: ""
                };
            ...
            calculateDetailHot.updateSettings({
                mergeCells: [
                    { row: lastRowIndex2, col: 1, rowspan: 1, colspan: 2 } // rptNo부터 description까지 열을 병합
                ]
            });
            ...
            calculateDetailHot.setCellMeta(lastRowIndex2, 3, 'className', 'boldText htRight');  // supAmt 열
            calculateDetailHot.setCellMeta(lastRowIndex2, 4, 'className', 'boldText htRight');  // taxAmt 열
            calculateDetailHot.setCellMeta(lastRowIndex2, 5, 'className', 'boldText htRight');  // totAmt 열
...
function fn_calculateTableCol(){
	...
	var paymentYnRenderer = function (instance, td, row, col, prop, value, cellProperties) {
		...
		this.calculateCol = [
			...
			{data : 'description', className : "htCenter", width: 150, className : "htCenter", readOnly:true},
			{data : 'supAmt', width: 150, type : 'numeric', className : "htRight", 	numericFormat : {pattern : '0,0'}, readOnly:true},
			{data : 'taxAmt', width: 150, type : 'numeric', className : "htRight", 	numericFormat : {pattern : '0,0'}, readOnly:true},
			{data : 'totAmt', width: 150, type : 'numeric', className : "htRight", 	numericFormat : {pattern : '0,0'}, readOnly:true},
			{data : 'remarks', className : "htCenter", width: 150, className : "htCenter", readOnly:true},
			{data : 'paymentYn', className : "htCenter", width: 150,wordWrap: false, readOnly:true, renderer : paymentYnRenderer}
			];
}
...
function fn_calculateDetailTableCol(){
	...
	this.calculateDetailCol = [
		...
		{data : 'partnCmpny', className : "htCenter", width: 100, className : "htCenter", readOnly:true},
		{data : 'blNo', className : "htCenter", width: 150, readOnly:true},
		{data : 'rptNo', className : "htCenter", width: 150, readOnly:true},
		{data : 'supAmt', width: 150, type : 'numeric', className : "htRight", 	numericFormat : {pattern : '0,0'}, readOnly:true},
		{data : 'taxAmt', width: 150, type : 'numeric', className : "htRight", 	numericFormat : {pattern : '0,0'}, readOnly:true},
		{data : 'totAmt', width: 150, type : 'numeric', className : "htRight", 	numericFormat : {pattern : '0,0'}, readOnly:true},
		{data : 'paymentYn', className : "htCenter", width: 150, readOnly:true,renderer : paymentYnRenderer},
		];
}
...
function fn_calculateTableHeader(){
		...
		/*if($("input:radio[name=calculate_srch1]:checked").val() == '01'){*/
			this.calculateHeader = [
				"B/LNo.", "신고번호", "신고일자","수리일자","구분", "적요", "공급가액", "VAT", "총합계", "비고", "대납여부"
				]; 
		/*}*/
...
function fn_calculateDetailTableHeader(){
	...
	this.calculateDetailHeader = [
		"구분", "B/LNo." , "신고번호" ,"공급가액", "VAT", "합계", "대납여부"
		]; 
}
	 ...
	 // columnSorting : {indicator : true},
      ...
      afterOnCellMouseDown : function(event, coords){
    	  if (coords.col === 4) {
    	        ...
    	        // Ensure dataList is valid
    	        if (dataList && dataList.length > 0) {
    	            var partnCmpny = dataList[dataList.length - 1].partnCmpny;
    	            console.log("partnCmpny: " + partnCmpny);

    	            // Call the function with partnCmpny
    	            fn_searchCalDetailView(partnCmpny);
    	        }
    	    }
      }
	};
...
function fn_changeCalculView(type){
	...
	var searchTp = $("input:radio[name=calculateViewType]:checked").val();
	if(searchTp == "edit"){
			$("#btnCalculateViewSave").show();
			/*$("#expExcel").hide();
...
function fn_changeCalculViewType(type){
	...
	if(tableType == "read"){
		...
		$("btnCalculateViewSave").hide();
			...
			calculateDetailHot2.loadData([]);
			calculateDetailHot2.alter('insert_row_below', 1, 1);
			//fn_loading(false);
		}if(tableType == 'edit'){
			calculateDetailHot2.loadData([]);
			fn_searchBankInsertView();
		}
...
function fn_calculateExcelDownload(button){
    ...
    //전체
    if(downloadType == '01'){
    	var calculateCol = new fn_calculateTableCol();
    	var calculateHeader = new fn_calculateTableHeader();
    	exCol = fn_getExcelCol(calculateCol.calculateCol); 
        exTit = fn_getExcelHead(calculateHeader.calculateHeader); 
        var calculateValue = calculateHot.getDataAtCell(1,0);
    ...
    //비용요약
    	...
    	exCol = fn_getExcelCol(calculateDetailCol.calculateDetailCol); 
        exTit = fn_getExcelHead(calculateDetailHeader.calculateDetailHeader); 
        var partnCmpnyColIndex = 0; // partnCmpny 열의 인덱스 (첫 번째 열)
        var partnCmpnyValue = calculateDetailHot.getDataAtCell(0, partnCmpnyColIndex);
        if(partnCmpnyValue == null){
            alert("데이터가 존재하지 않습니다.");
            return;
        }
   //이체정보
    	...
    	exCol = fn_getExcelCol(calculateDetailCol2.calculateDetailCol2); 
        exTit = fn_getExcelHead(calculateDetailHeader2.calculateDetailHeader2); 
        var calculateDetailValue = calculateDetailHot2.getDataAtCell(0,0);
    ...
    $("#calculateExcelForm").append("<input type='hidden' name='searchType' value="+ partnCmpnyValue +">");
