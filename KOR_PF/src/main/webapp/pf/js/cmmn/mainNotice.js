...
function fillModalContent(resultList) {
    ...
    var strHtml = "";
    if (notice.fileOrgNm) {
        strHtml = '<a href="#" id="noticeDownload" style="color:#030303"><i class="fa-regular fa-floppy-disk"></i> 첨부파일 : ' + notice.fileOrgNm + '</a>';
    }
    ...
    var noticeDownloadLink = document.getElementById('noticeDownload');
    if (notice.fileOrgNm) {
...
// 첨부파일
$(document).on('click', '#noticeDownload', function(){
	
	const htmlContent = event.target.innerHTML.trim();
	const fileOrgNm = htmlContent.split(' 첨부파일 : ')[1];
    $("#noticeFileDown").val(fileOrgNm);
    document.noticeFileDownForm.action = "/cmmn/noticeFileDown.do";
    document.noticeFileDownForm.submit();
})
