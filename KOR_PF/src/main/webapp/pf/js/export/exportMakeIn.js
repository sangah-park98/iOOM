...
function fn_callTempInvoiceData(parameterValue1) {
	// console.log("parameterValue1 : " , parameterValue1);
	var sData = {};
	...
	$.ajax({
        ...
        success : function(data) {
           ...
           for (var i = 0; i < data.resultList2.length; i++) {
              ...
              console.log("nation: " + data.resultList2[i].nation);
	...
	$.ajax({
        ...
        success : function(data) {
        	if (data.resultList != null && data.resultList2 != null) {
	            ...
	            var totalCnt = parseInt(data.totCnt) + parseInt(data.totCnt2);
	            $("#exportInCnt").text(totalCnt.toLocaleString());

	            exportInHot.loadData([]);
	            exportInHot.loadData([]);
	            exportInHot.loadData(mergedResult);
				fn_loading(false);
        	}
        },
        error : function(e, textStatus, errorThrown) {
...
function fn_saveExportInCheck(){
	...
	console.log(exportInHot.getData());
	fn_validateExportIn();
};
