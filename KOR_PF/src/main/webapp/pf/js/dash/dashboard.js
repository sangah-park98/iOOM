...
function filldashNewsModalContent(resultList) {
    ...
    if (news.fileName && news.fileName.trim() !== '') {
        ...
        strHtml = '<i class="fa-regular fa-floppy-disk"></i> 첨부파일 :<br>' + strHtml;
    }
...
function exportRptPdf(data){
	...
	if (!window.jsPDF) window.jsPDF = window.jspdf.jsPDF;
        ...
        // ✅ 한글 폰트 추가 (malgun.ttf 필요)
        doc.addFont('/pf/fonts/NanumSquare.ttf', 'NanumSquare', 'normal');
        doc.setFont('NanumSquare'); // 사용하려는 폰트 지정
        

        var img = new Image();
        //img.src = '/pf/images/report_2.jpg'; //파란
        ...
        img.onload = function () {
           ...
           // doc.addImage(ioomLogoImg, 'PNG', 181, 253, 14,5);
            ...
            doc.text(105, 120, customsReport, { align: "center" }); // 통관 리포트 customsReport
            doc.setFontSize(30);
            ...
            var reportData1 = data.reportData1;  // data.reportData1이 제공된다고 가정
	            ...
	            var datas = reportData1.map(item => {
	              ...
	              return [
	                item.lisDay,        // 수리일자
	                item.cnt,           // 건수
	                formattedAmountKrw, // 과세가격 (포매팅된 값)
	                item.txType1,       // 일반수입
	                item.txType2,       // 수리물품
	                item.txType3,       // 견품 및 광고용품
	                item.txType4,       // 기타 수입 승인면세
	                item.txType5,       // 재수출조건
	                item.txPrc1,        // P/L
	                item.txPrc2,        // 제출서류
	                item.txPrc3,        // 검사
		       ...
		       var header = [
		    	   	[ 
		    		    ...
		    		    { content: '과세가격(KRW)', rowSpan: 2, border: [true, true, true, true] },
		    		    { content: '수입 거래형태 현황', colSpan: 5, border: [true, true, true, true] },  // 수입 거래형태 현황 셀에 테두리 추가
			   ...
			   // 🟢 테이블 스타일 적용
	                ...
	                reportData3.forEach(item => {
	                  ...
	                  if (year === firstYear) {
	                	...
	                	// 올해
	                    lastYearData.push(item.cntImp);  // cntImp 값을 넣음
	                  }
	                });
	            	...
	            	pdf2canvas.style.display = 'none'; // 웹 화면에서 숨김 처리
               ...
               /* doc.setLineWidth(0.5);  // 테두리 두께 설정 (기본값은 0.2)
            	...
            	doc.autoTable({
            	  ...
            	  columnStyles: {
            		  	0: { halign: 'center'}, // 랭크
            	        1: { halign: 'center'}, // 세번부호
            	        2: { halign: 'center'}, // 관세구분
            	        3: { halign: 'center'}, // 품명
            	        4: { halign: 'center'}, // 관세율
            	        5: { halign: 'right'}, // 과세가격
            	        6: { halign: 'right'}, // 관세액
            	...
            	if(spaceLeft3< 130){
            		...
            		chartY3 = 15;
            		 ...
            		 chartPage3 = pageAfterTable3;       // 같은 페이지에 그릴 수 있음
            		  chartY3 = lastYPage3;
            	}
               ...
               console.log("reportData8",reportData8);
                ...
                var reportData9 = data.reportData9;  
                   ...
                   if(reportData10.length == 0){
                	...
                	if(spaceLeft7< 120){
                		...
                		chartY7 = 15;
                		 ...
                		 chartPage7 = pageAfterTable7;       // 같은 페이지에 그릴 수 있음
                		  chartY7 = lastYPage7;
                	}
                	...
                	var pdf8chart = new Chart(pdf8ctx, {
                		plugins : [chartDataLabels4],
    	            	    ...
    	            	    options: {
    	            	    	 responsive: false, // ✅ 차트 크기 자동 조절 비활성화
    	            	         maintainAspectRatio: true, // ✅ 원형 비율 유지 비활성화
    	            	        layout: {
    	            	        	padding: {
    	            	                top: 300,
    	            	                right: 320,
                   ...
                   var datas11 = reportData12.map(item => {
                     ...
                     return [
                       item.incoterms,  // 인코텀즈
                       item.impCnt,     // 수입 건수
                       formattedlmpAmt, // 수입 금액
                       impRate,         // 수입 비율
                       item.expCnt,     // 수출 건수
                       formattedEmpAmt, // 수출 금액
                       expRate          // 수출 비율
                     ];
                   });
                ...
                // 테이블 헤더
    		       var header10 = [
                	...
                	doc.autoTable({
                	  head: header10,
                	  body:datas11,
                	  startY: lastYPage8 + 29, // 설명 아래에 테이블 배치
                	  theme: 'grid',
                	  styles: {
                	    font: 'NanumSquareR',
                	    fontSize: 7,
	            ...
	            setTimeout(() => {
            	    ...
            	    for (let i = 3; i <= totalPages; i++) {
            	        ...
            	        doc.setFontSize(9);
            	        doc.setTextColor(0, 0, 0);
            	        doc.text(`-${i-2}-`, 105, 290, { align: "center" }); // 중앙 정렬
            	    }
