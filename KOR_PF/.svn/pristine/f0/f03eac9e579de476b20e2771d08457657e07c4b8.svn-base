...
<head>
  ...
  <script src="/pf/js/rpt/fundSettle.js?v=<%=fmt.format(today)%>" charset="UTF-8"></script>
  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.4/xlsx.full.min.js"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.17.4/xlsx.full.min.js"></script>
<script type="text/javascript">
	//세관
	var customsList = JSON.parse('${customsList}');
	//포워더
	var forwordList = JSON.parse('${forwordList}');
	//운송
	var transportationList = JSON.parse('${transportationList}');
	//창고
	var warehouseList = JSON.parse('${warehouseList}'); 
	//보험료
	//var PremiumList = JSON.parse('${PremiumList}'); 
	var PremiumList = JSON.parse('${premiumList}' || '{}'); 
	
	//통관수수료
	var feeList = JSON.parse('${feeList}'); 
	
	//모든 리스트
	var masterList = JSON.parse('${masterList}'); 
...
<main class="p-2 grow flex flex-col gap-2">
  <form
    ...
    <div class="row-start-1 col-span-6 col-end-13 flex flex-row gap-2 items-end justify-end">
      ...
      <button 
      	type="button"
        onclick="fn_searchFundSettleViewInfo()"
      ...
      <button
      	type="button"
      	onclick="fn_clearfundSettleView()"
    ...
    <!-- 검색기간 -->
    <label
      class="registerDay col-start-1 row-start-2 col-span-1 row-span-1 flex items-center font-medium text-gray-900 pr-2"
    >
      등록일자
    </label>
    <select
      id="fundSettle_day"
      name="fundSettle_day"
      class="registerDay row-start-2 col-start-2 col-span-1 bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-500 focus:border-primary-500 block py-1 px-2.5">
    ...
    ">
      <div class="relative grow">
        ...
        <input type="text" class="form-control band-calendar text-base bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full ps-10 py-1 px-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
			id="fundSettle_srch2" name="fundSettle1_date"
      ...
      <div class="relative grow">
        ...
        <input type="text" class="form-control band-calendar text-base bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full ps-10 py-1 px-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
				id="fundSettle_srch3" name="fundSettle2_date"
    ...
    <div id="fundSettle_div2" class="col-start-7 row-start-2 col-span-4 flex items-center gap-1">
      <button
          type="button"
    ...
    <!-- 신고번호 -->
    <label for="n2" class="fundSettle_div3 row-start-3 col-start-1 flex items-center font-medium text-gray-900 pr-2">
      	신고번호
    </label>
    <input
      type="text"
      id="fundSettle_srch6"
      onkeyup="enterkey()"
      class="fundSettle_div3 row-start-3 col-start-2 col-span-5 bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-500 focus:border-primary-500 block py-1 px-2.5"
      placeholder="신고번호를 입력해주세요."
    >
    <!-- B/L 번호 -->
    <label for="n3" id="blNoLabel" class="fundSettle_div4 row-start-3 col-start-7 flex items-center font-medium text-gray-900 px-2">
      B/L 번호
    </label>
    <input
      type="text"
      id="fundSettle_srch7"
      onkeyup="enterkey()"
      class="fundSettle_div4 row-start-3 col-start-8 col-span-5 bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-500 focus:border-primary-500 block py-1 px-2.5"
      placeholder="B/L번호를 입력해주세요."
    >
    
     <!-- 신고번호 -->
    <label for="n2" class="fundSettle_div5 row-start-3 col-start-1 flex items-center font-medium text-gray-900 pr-2">
      	코드
    </label>
    <input
      type="text"
      id="fundSettle_srch8"
      onkeyup="enterkey()"
      class="fundSettle_div5 row-start-3 col-start-2 col-span-5 bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-500 focus:border-primary-500 block py-1 px-2.5"
      placeholder="코드를 입력해주세요."
    >
    <!-- B/L 번호 -->
    <label for="n3"  class="fundSettle_div6 row-start-3 col-start-7 flex items-center font-medium text-gray-900 px-2">
     	이름
    </label>
    <input
      type="text"
      id="fundSettle_srch9"
      onkeyup="enterkey()"
      class="fundSettle_div6 row-start-3 col-start-8 col-span-5 bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-500 focus:border-primary-500 block py-1 px-2.5"
  ...
  <div class="flex items-center justify-between w-full shrink-0">
    <div class="flex items-center gap-4">
      <h2 id="fundSettleTextView" class="shrink-0 flex items-center gap-1">
      </h2>
      <p class="card-title inline" style="line-height: 39px">결과: <span id="fundSettleCnt">0</span>
      </p>
	      <!-- 라디오 버튼 그룹 -->
    <div class="flex items-center gap-2">
      <label for="default-radio-7" class="flex items-center gap-1">
        <input id="default-radio-7" type="radio" name=fundSettleType value="read" checked
          class="w-4 h-4 text-primary-600 bg-gray-100 border-gray-300 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
        <span>읽기</span>
      </label>
      <label for="default-radio-9" class="flex items-center gap-1">
        <input id="default-radio-9" type="radio" name="fundSettleType" value="edit"
          class="w-4 h-4 text-primary-600 bg-gray-100 border-gray-300 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
        <span>편집</span>
      </label>
      <label for="default-radio-8" class="flex items-center gap-1">
        <input id="default-radio-8" type="radio" name="fundSettleType" value="enrol"
          class="w-4 h-4 text-primary-600 bg-gray-100 border-gray-300 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
        <span>등록</span>
      </label>
    </div>
...
<!--       <button
      ...
      >
       ...
       -->
            ...
            <button
		        type="button"
		        id="fundSettleSaveBtn" onclick="fn_savefundSettleSave()"
		      ...
		      >
		       ...
		       <button
		        type="button"
		        id="fundSettleEtcSaveBtn" onclick="fn_savefundSettleEtc()"
		        class="upload-button h-9 text-white bg-secondary-700 hover:bg-primary-800 font-medium rounded px-2.5 py-2 text-base border border-secondary-700 hover:border-primary-400"
		      >
		        <i class="fa-regular fa-floppy-disk"></i>
		        저장
         ...
         <button
      	 type="button"
      	 onclick="fn_funcSettleExcelDownload()"
      	 data-download-type="01"
         id="fundSettleExcelDown"
      ...
      <select 
        id="fundSettlePageCnt"
      	name="fundSettlePageCnt"
  ...
  <!-- MainTable -->
  <div id="fundSettleTable"
       class="grow bg-white shadow-sm h-full  rounded-lg border border-slate-200 z-0">
  </div>

<!-- detail modal -->
<div id="fundSettleDetailtPopUp" class="modal fade fixed top-0 left-0 h-full w-full z-[200] bg-black/50 items-center justify-center duration-300" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
	...
	<div class="modal-dialog modal-xl" style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); width:800px;" >
		<div class="modal-content bg-white shadow-lg relative rounded min-w-80 overflow-hidden">
			<div class="pl-7 pr-5 py-2 text-white bg-primary-900 flex items-center justify-between">
				<h1 class="fundSettleModal-title">
					<span></span>
				</h1>
				<button type="button" onclick="fundSettleDetailListClose()" class="modal-close text-2xl px-1.5 py-1 rounded-lg hover:bg-rose-500/70 border-2 border-transparent hover:border-white duration-300 flex items-center justify-center"><i class="far fa-xmark"></i></button>
			</div>
			<div class="pl-7 pr-5 py-2 flex items-center justify-end">
			    <button type="button" onclick="fn_fundSettleDetailListSave()"  class="upload-button h-9 text-white bg-secondary-700 hover:bg-primary-800 font-medium rounded px-2.5 py-2 text-base border border-secondary-700 hover:border-primary-400">
		        <i class="fa-regular fa-floppy-disk"></i>&nbsp;저장
		        </button>
			</div>
			<div class="modal-body">
				<div id="fundSettlePopupTable" class="grow bg-white shadow-sm h-full max-h-[100rem] rounded-lg border border-slate-200 z-0"></div>
