	...
	private pfItemService pfitemService;
		...
		public ModelAndView selectItemViewList(@ModelAttribute("searchVO") SearchVO vo, HttpServletRequest request, ModelMap model) throws Exception {
		     ...
		     if(vo.getExType().equals("01")) {
	    	   ...
	    	   if(vo.getSrch13()!= null) { //중복허용할때
		    	   ...
		    	   if(vo.getSrch13()!= null) { //중복허용
					   ...
					   totCnt = pfitemService.selectDuplicationItemViewExportListCnt(vo); 
		    	   }else {//중복허용x
		    		   System.out.println("수출,중복X");   
					   resultList = pfitemService.selectItemViewExportList(vo);
					   totCnt = pfitemService.selectItemViewExportListCnt(vo); 
		    	   }
		   ...
		   /* if (vo.getPageIndex() != 0) {
		        int start = (vo.getPageIndex()*vo.getRecordCountPerPage())+1;
		        int end = (start + vo.getRecordCountPerPage())-1; 
		        vo.setStartPageMs(start);
		        vo.setRecordCountPerPage(end);
			       if(vo.getExType().equals("01")) {
			    	   
			    	   if(vo.getSrch13()!= null) {// 중복허용할때
			    		   System.out.println("전체,중복O");
			    		   resultList = itemService.selectDuplicationItemViewList(vo);
							totCnt = itemService.selectDuplicationItemViewListCnt(vo);
			    	   }else { // 중복 허용 X
			    		   System.out.println("전체,중복X");
			    		    resultList = itemService.selectItemViewList(vo);
							totCnt = itemService.selectItemViewListCnt(vo);
			    	   }
			    	   ...
			    	   if(vo.getSrch13()!= null) { //중복허용할때
			    		   System.out.println("수입,중복O"); 
			    		   resultList = itemService.selectDuplicationItemViewimportList(vo);
			    		   totCnt = itemService.selectDuplicationItemViewimportListCnt(vo);
			    	   }else { //중복허용x
			    		   System.out.println("수입,중복X");
			    		   resultList = itemService.selectItemViewimportList(vo);
			    		   totCnt = itemService.selectItemViewimportListCnt(vo);
			    	   }
						
			       }else if(vo.getExType().equals("03")) {
			    	   if(vo.getSrch13()!= null) { //중복허용
			    		   System.out.println("수출,중복O"); 
						   resultList = itemService.selectDuplicationItemViewExportList(vo);
						   totCnt = itemService.selectDuplicationItemViewExportListCnt(vo); 
			    	   }else {//중복허용x
			    		   System.out.println("수출,중복O");   
						   resultList = itemService.selectItemViewExportList(vo);
						   totCnt = itemService.selectItemViewExportListCnt(vo); 
			    	   }
		        ...
		        // 처음로딩될때
		    	 if(vo.getExType().equals("01")) {
		    		 if(vo.getSrch13()!= null)  {// 중복허용할때
			    		   System.out.println("전체1,중복O");
			    		   resultList = itemService.selectDuplicationItemViewList(vo);
							totCnt = itemService.selectDuplicationItemViewListCnt(vo);
			    	   }else { // 중복 허용 X
			    		   System.out.println("전체1,중복X");
			    		    resultList = itemService.selectItemViewList(vo);
							totCnt = itemService.selectItemViewListCnt(vo);
			    	   }
			       }else if(vo.getExType().equals("02")) {
		...
		public ModelAndView selectItemViewLanList(@ModelAttribute("searchVO") SearchVO vo, HttpServletRequest request, ModelMap model) throws Exception {
			    ...
			    /*if (vo.getPageIndex() != 0) {
			        ...
			        System.out.println("strt2 : " + vo.getStartPageMs()); 
			        System.out.println("end2 : " + vo.getRecordCountPerPage());
			        System.out.println("INDEX2 : " + vo.getPageIndex());

			        if(vo.getExType().equals("01")) {
							System.out.println("전체");
							resultList = itemService.selectItemViewLanList(vo);
				       }else if(vo.getExType().equals("02")) {
							System.out.println("수입"); 
							resultList = itemService.selectItemViewLanImportList(vo);
				       }else if(vo.getExType().equals("03")) {
							System.out.println("수출");  
							resultList = itemService.selectItemViewLanExportList(vo);
				       }
		...
		public ModelAndView selectitemViewLanList2(@ModelAttribute("searchVO")SearchVO vo,HttpServletRequest request,ModelMap model)throws Exception{
		    ...
		    if(vo.getExType().equals("01")) {
				...
				totCnt = pfitemService.selectItemViewLanListCnt(vo);
				System.out.println("totCnt2"+totCnt);
	       }else if(vo.getExType().equals("02")) {
				System.out.println("수입1");  
		...
		try {
			...
			divUnion = vo.getExTitDiv().split("\\|\\|", -1);
		     ...
		     System.out.println("divUnion: " + Arrays.toString(divUnion));
