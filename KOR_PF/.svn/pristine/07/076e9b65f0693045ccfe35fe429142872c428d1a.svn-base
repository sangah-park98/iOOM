...
$( document ).ready(function() {
	  ...
	  //scroll 이벤트
	  fn_expInasEventReg();
	  $("#exportIn_div6 p").html(
		'<i class="fa-duotone fa-feather text-primary-900"></i>&nbsp;&nbsp;' +
		 totalCnt + ': <span id="exportInCnt">0</span>'
	  );
	  ...
	  if(corpNos != 1){
		 ...
		 $("#exportMakeIn").hide();
		 $("#exportMakeIn2").hide();
		 $("#uploadInSaveBtn").hide();
		 $("#dropzone-file-expUpload").hide();
		 $("#uploadInDiv").hide();
	  }
});
...
function fn_handsonGridinListPopupOption() {
    inListPopupSettings = {
        columns: [
            ...
            { data: 'docuOrgFile', type: 'text', width:280, className: "htCenter", readOnly: true },
            { data: 'docuFile', type: 'text', className: "htCenter", readOnly: true },
            { data: 'uploadDt', type: 'text', className: "htCenter", width: 50, readOnly: true },
            {
        ...
        colHeaders: ["", invoiceNo, fileType, fileName, "", uploadDt, del, ""],
        /*colHeaders: ["", "INVOICE 번호", "파일 타입", "파일명", '', "업로드 일자", "삭제", ''],*/
...
function fn_searchExportIn(){
	var selectedValue = $("input[name=exportIn_srch1]:checked").val();
    var text = "";
    ...
    $("#exportIn_div6 p").html(
	    '<i class="fa-duotone fa-feather text-primary-900"></i></i>&nbsp;&nbsp;' +
	    text + ': <span id="exportInCnt">0</span>'
	);
...
function docuFileLoadRenderer(instance, td, row, col, prop, value, cellProperties) {
    var $fileInButton;
    var data = exportInHot.getData();
    var valueYn = data[row] && data[row][1] !== undefined ? data[row][1] : '';
    var valueYn1 = data[row] && data[row][2] !== undefined ? data[row][2] : '';
    var valueYn2 = data[row] && data[row][6] !== undefined ? data[row][6] : '';
        
	if(valueYn === '임시저장'){}
	else {
	  if (value != '' && value != null) {
		  $fileInButton = $('<i class="fas fa-search search-icon" style="cursor:pointer;" onclick="fn_inFileList('+row+','+col+')"></i>');
      } else {
    	if(corpNos == 1){
    		 $fileInButton = $('<i class="fas fa-plus search-icon" style="cursor:pointer;" onclick="fn_fileUpDownIn('+row+','+col+')"></i>');
    	}
      }
	}
    $(td).empty().append($fileInButton).append("  " + value);
};

//통관의뢰 버튼 업로드
function expRenderer(instance, td, row, col, prop, value, cellProperties) {
    ...
    var valueYn1 = data[row] && data[row][2] !== undefined ? data[row][2] : '';
    var valueYn2 = data[row] && data[row][6] !== undefined ? data[row][6] : '';
    var valueYn3 = data[row] && data[row][7] !== undefined ? data[row][7] : '';
    ...
    if (valueYn3 === 'N') {
    	if (valueYn === '' && valueYn1 === '' && valueYn2 != '') {
	    	...
	    	$fileButton.css({'float': 'right'});
	        $(td).empty().html('<div>' + value + "  " + $fileButton[0].outerHTML + '</div>');
	} else if(valueYn1 ==='임시저장') {
		var $fileButton = $('<button type="button" class="chip chip-blue" onclick="fn_callTempInvoice(' + row + ',' + col + ')">불러오기</button>');
    	$fileButton.css({'float': 'right'});
        $(td).empty().html('<div>' + value + "  " + $fileButton[0].outerHTML + '</div>');
	} else {
...
function chipRenderer(instance, td, row, col, prop, value, cellProperties) {
...
function fn_expInTableCol(){
	...
	// 문서 다운로드 업로드
	this.expInCol = [
		{data: 'type', className: "htCenter", wordWrap: false, width: 110,  readOnly: true, renderer: expRenderer},
	    {data: 'invoiceNo', className: "htCenter", wordWrap: false, readOnly: true}, // [ref] width: 150
	    {data: 'rece', className: "htCenter", wordWrap: false, readOnly: true, renderer: chipRenderer},
	    {data: 'rptNo', className: "htCenter", wordWrap: false, readOnly: true},
	    {data: 'rptDay', className: "htCenter", wordWrap: false, readOnly: true},
	    {data: 'expLisDay', className: "htCenter", wordWrap: false, readOnly: true},
	    {data: 'pdf', className: "htCenter", wordWrap: false, readOnly: true, renderer: docuFileLoadRenderer},
	    {data : 'requestYn', className : "htCenter", wordWrap: false, className : "htCenter", readOnly:true, renderer : expRenderer},
	];
}
...
function fn_expInTableHeader(){
	var tableType = $("input:radio[name=exportInType]:checked").val();
	this.expInHeader = [
		type, invoiceNo, status, rptNo, rptday, lisDay, pdf, ""
		/*"구분", "INVOICE 번호", "상태", "신고번호", "신고일자", "수리일자", "PDF", ""*/
	] ;
}
...
function fn_handsonGridReadViewOption(col, header, hidden){
	...
	exportInSettings = {
	  ...
	  allowRemoveRow : true,
	 // columnSorting : {indicator : true},
      autoColumnSize : {samplingRatio : 23, syncLimit: 300},
      mergeCells : false,
      wordWrap : true,
...
function fn_changeExportIn(type){
	...
	fn_changeExportInType();
};

//검색구분 변경
function fn_changeExportInType(){
	let expInCol = new fn_expInTableCol();
	let expInHeader = new fn_expInTableHeader();
	let expInHidden = new fn_expInTableHidden();
	var col, header, hidden;

	col = expInCol.expInCol;
	header = expInHeader.expInHeader;
	hidden = expInHidden.expInHidden;
...
function fn_fileInSave(){
	...
	if (fileList.length === 0 || files.length === 0) {
        alert(msgUploadFile); // '파일을 업로드해 주세요.'
        return;
    }
	...
	for (let i = 0; i < loadedFilesIn.length; i++) {
        ...
        if (matchingLiData) {
            ...
            if (fileNameWithoutExtension.startsWith('CI_') && matchingLiData.fileType === 'bl') {
                alert(msgCiFileType); // '"CI_"로 시작하는 파일은 파일 타입이 INVOICE이어야 합니다.'
                return;
            }
	...
	formData.append('rptNo', $("#rptNo").val() );
 	...
 	if (fileInEntries.length !== 1) {
 	    alert(msgCiFileOne); // 'INVOICE 파일은 반드시 한 개가 등록되어야 합니다.'
 	    return;
 	} else {
 	    // 파일 확장자 제거
 	    var fileNameWithoutExtension = fileInName.split('.').slice(0, -1).join('.');

 	    if (!(fileNameWithoutExtension.startsWith('CI_') || fileNameWithoutExtension.startsWith('ci_')) && $("#invoiceNo").val() == "") {
 	        alert(msgFileStartToCi); // 'INVOICE의 파일명은 "CI_"로 시작되어야 합니다.'
 	        return;
 	    }
 	    if (fileNameWithoutExtension.startsWith('CI_') && (fileNameWithoutExtension.replace('CI_', '').trim() === '') && $("#invoiceNo").val() == "") {
 	        alert(msgFileInputCiNo); // 'INVOICE의 파일명은 "CI_" 뒤에 INVOICE 번호를 기재해야 합니다.'
 	        return;
 	    } 
 	}
   ...
   for(var i=0; i<data.length; i++){
    	var blno = data[i]["pdf"];
		if(blno == fileInName){
			alert(msgFileAlreadyCiDone); // '해당 INVOICE 번호로 이미 파일이 등록되어 있습니다.'
			return;
		}
   }  
...
function fn_docuPrint() {
     ...
     if (cnt == 1) {
         ...
         document.zipInDownForm.action = "/base/downloadFile.do";
...
function fn_expPopupDelete () {
    var rowDataIn = inListHot.getSourceData();
    var invoiceNo = rowDataIn[0].invoiceNo;
    
    let itemsToDeleteIn = [];
    var cnt = 0;
    for (let i = 0; i < rowDataIn.length; i++) {
        if (rowDataIn[i].checkBox === "yes") {
        	itemsToDeleteIn.push(rowDataIn[i]);
        	cnt++;
        }
    }
    if (cnt == 0){
    	alert(msgNoSelectedFile); // "선택된 파일이 없습니다."
    	return;
    }
    if (confirm(msgDelSelectedFile)) { // "선택한 파일을 삭제하시겠습니까?"
        $.ajax({
        	type: "POST",
            url: "/export/deleteExpInFile.do",
            data: JSON.stringify(itemsToDeleteIn), 
            contentType: 'application/json', 
            beforeSend: function(xhr) {
                xhr.setRequestHeader("AJAX", "true");
            },
            success: function(data) {
            	alert(msgFileDel); // '파일이 삭제되었습니다.'
            	var xData = {};
            	xData["srch1"] = invoiceNo;
            	fn_searchInFilesPopup(xData);
              },
