...
function fn_searchItemView(){
	...
	$.ajax({
        ...
        success : function(data) {
            	...
            	$("#itemViewCnt").text(data.totCnt.toLocaleString()); //검색결과 총 갯수
        	fn_loading(false);
        },
        error : function(e, textStatus, errorThrown) {
        	if(e.status == 400){
...
function fn_searchItemViewLan2(){
	...
	$.ajax({
        ...
        success : function(data) {
        		...
        		$("#itemlanViewCnt").text(data.totCnt.toLocaleString()); //검색결과 총 갯수
        		itemViewLanHot.loadData([]);
        		itemViewMemoHot.loadData([]);
...
function fn_saveItemViewMemoCheck(){
	...
	console.log("memo"+JSON.stringify(saveMemoCheckData));
...
function fn_saveItemViewSpecCheck(itemcode,rptNo,ran,sil){
	...
	console.log("spec"+JSON.stringify(saveSpecCheckData));
...
function fn_saveItemViewCheck(){
	...
	console.log(itemViewHot.getData());
	fn_validateItemView();
};
...
function fn_itemViewLanTableHeader(){
	...
	this.itemViewLanHeader = (searchTp == "01" && (tableType == "read" || tableType == "edit")) ? [
		...
		"세종","세율","수량","단위","단가","원산지표시유무","자재코드","공장코드","PO","B/L","SO","Invoice","result"
	] : (searchTp == "02" && (tableType == "read" || tableType == "edit")) ? [
		"신고번호","란","규격","신고일자","수리일자","HSCODE",
		"세종","세율","수량","단위","단가","원산지표시유무","자재코드","공장코드","PO","B/L"
	] : (searchTp == "03" && (tableType == "read" || tableType == "edit")) ? [
		"신고번호","란","규격","신고일자","수리일자","HSCODE",
		"세종","세율","수량","단위","단가","원산지표시유무","자재코드","공장코드","SO","Invoice"
	]: [
		"신고번호","란","규격","신고일자","수리일자","HSCODE",
		"세종","세율","수량","단위","단가","원산지표시유무","자재코드","공장코드","PO","B/L","SO","Invoice"
	];
}
...
function fn_itemViewExcelDownload(){
   ...
   $("#itemViewExcelForm").append("<input type='hidden' name='exTitDiv' value="+ exTitDivArr +">");
