...
$( document ).ready(function() {
	...
	$("#tradeInfo_div6 p").html(
		'<i class="fa-duotone fa-chart-network text-primary-900"></i>&nbsp;&nbsp;' +
		korExpResultCnt + ': <span id="tradeInfoCnt">0</span>'
	);
});
...
$("input[name=tradeInfo_srch1]").change(function(){
    ...
    if(selectedValue === "01") {
    	...
    	$("#tradeInfo_div6 p").html(
			'<i class="fa-duotone fa-chart-network text-primary-900"></i>&nbsp;&nbsp;' +
			VnmImpResultCnt + ': <span id="tradeInfoCnt">0</span>'
    	);
        fn_changeTradeViewType(selectedValue);
    } else if(selectedValue === "03") {
    	$("#tradeInfo_div6 p").html(
			'<i class="fa-duotone fa-chart-network text-primary-900"></i>&nbsp;&nbsp;' +
			sehyVnResultCnt + ': <span id="tradeInfoCnt">0</span>'
    	);
        fn_changeTradeViewType(selectedValue);
    }
});
...
function fn_tradeViewTableCol(){
	...
	if (tradeType == "01"){
		this.tradeViewCol = [
			...
			{data : 'expLisDay', className : "htCenter", width: lang === 'en' ? 120 : 90, wordWrap: false, className : "htCenter", readOnly:true},
			...
			{data : 'tradeQty', type : 'numeric', width: lang === 'en' ? 125 : 90 , className : "htRight" , 	numericFormat : {pattern : '0,0'},readOnly : true},
			{data : 'tradeRemark', className : "htCenter", width: 200, wordWrap: false, className : "htCenter", readOnly:true},
			{data : 'regDt', className : "htCenter", width: lang === 'en' ? 120 : 90, wordWrap: false, className : "htCenter", readOnly:true},
			{data : 'regId', className : "htCenter", width: 75, wordWrap: false, className : "htCenter", readOnly:true},
		] ;
		...
		this.tradeViewCol = [
			...
			{data : 'regDt', className : "htCenter", width: lang === 'en' ? 120 : 90, wordWrap: false, className : "htCenter", readOnly:true},
			{data : 'regId', className : "htCenter", width: 70, wordWrap: false, className : "htCenter", readOnly:true},
			] ;
	} else {
		this.tradeViewCol = [
			...
			{data : 'tradeQty', type : 'numeric', width: lang === 'en' ? 120 : 90 , className : "htRight" , 	numericFormat : {pattern : '0,0'},readOnly : true},
			{data : 'tradeRemark', className : "htCenter", width: 180, wordWrap: false, className : "htCenter", readOnly:true},
			{data : 'regDt', className : "htCenter", width: lang === 'en' ? 120 : 90, wordWrap: false, className : "htCenter", readOnly:true},
			{data : 'regId', className : "htCenter", width: 70, wordWrap: false, className : "htCenter", readOnly:true},
		] ;
	}
...
function fn_tradeViewTableHeader(){
	...
	var chkTradeType = $("input:radio[name=tradeInfo_srch1]:checked").val();
	// 사용여부
	//var tradeInfo_srch20 = $("input:radio[name=tradeInfo_srch20]:checked").val(); 
	if(chkTradeType == "01"){
	this.tradeViewHeader = [
		expRptNo, lisDay, tradeType, goodsName, specItemCd, qty,
		unit, invoiceNo, overseas, revisionQty, revisionReason, revisionDt, register
		 // "수출신고번호", "수리일자", "거래구분", "거래품명", "규격(ITEM CODE)" , "수량" , 
		 // "단위" ,"인보이스 번호", "해외거래처",  "보정 수량", "보정 사유", "보정 일자", "등록자"
	 ] ;
	...
	this.tradeViewHeader = [
		regDt, transactionNo, wareHousingType, uniqueVal, tariffCd, customsDeclareCd, carryInDt, unit,
		qty, tradeType, invoiceNo, revisionQty, revisionReason, revisionDt, register
		// "등록일자", "거래번호", "창고구분", "고유값", "품목코드(ITEM CODE)", "세관신고코드", "입고일자", "단위" , 
		// "수량" , "거래구분", "인보이스 번호",  "보정 수량", "보정 사유", "보정 일자", "등록자"
	] ;
	}
}
...
// table
function fn_handsonGridTradeViewOption(col, header, hidden){
	var tableType = $("input:radio[name=tradeInfoType]:checked").val();
...
function fn_changeTradeViewType(type){
	...
	tradeInfoHot.updateSettings(fn_handsonGridTradeViewOption(col, header, hidden));
	fn_searchTradeView();
};
...
function fn_tradeInfoExcelDownload(){
	 ...
	 fn_loading(true);
	   ...
	   $.each(fn_setTradeViewForm(), function(attrName, attrValue){
		...
		parameters.srch40 = "거래정보관리";
		
		$.ajax({
			 url: "/trade/tradeViewExcel.do",
			 data: parameters,
			 type: 'POST',
			 cache: false,
			 timeout: 200000,
