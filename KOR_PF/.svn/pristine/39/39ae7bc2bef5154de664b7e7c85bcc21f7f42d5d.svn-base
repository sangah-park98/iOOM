...
$( document ).ready(function() {
	  ...
	  $(".export-view").hide();
      $(".import-view").show();
      $("#deadLine_div6 p").html(
		'<i class="fa-duotone fa-feather text-primary-900"></i>&nbsp;' +
		 impCnt2 + ': <span id="deadLineViewCnt">0</span>'
      );
	  //scroll 이벤트
	  ...
	  $(document).on("click", '.itemCdClass', function(){
		...
		$("#deadLineSrchType1").append('<option value="">'+ select +'</option>');
		$("#deadLineSrchType1").append('<option value="nab_firm">'+ nabFirm +'</option>');
		$("#deadLineSrchType1").append('<option value="levForm">'+ paymentType +'</option>');
		$("#deadLineSrchType1").append('<option value="tgPlnMark">'+ entryCharMark +'</option>');
		$("#deadLineSrchType1").append('<option value="tgPlnCot">'+ entryPlan +'</option>');
		$("#deadLineSrchType1").append('<option value="plantCd">'+ factoryCd +'</option>');
		$("#deadLineSrchType1").append('<option value="po">'+ po +'</option>');
		$("#deadLineSrchType1").append('<option value="reporter">'+ reporter +'</option>');
		
		$("#deadLineSrchType2 option").remove();
		$("#deadLineSrchType2").append('<option value="">'+ select +'</option>');
		$("#deadLineSrchType2").append('<option value="nab_firm">'+ nabFirm +'</option>');
		$("#deadLineSrchType2").append('<option value="levForm">'+ paymentType +'</option>');
		$("#deadLineSrchType2").append('<option value="tgPlnMark">'+ entryCharMark +'</option>');
		$("#deadLineSrchType2").append('<option value="tgPlnCot">'+ entryPlan +'</option>');
		$("#deadLineSrchType2").append('<option value="plantCd">'+ factoryCd +'</option>');
		$("#deadLineSrchType2").append('<option value="po">'+ po +'</option>');
		$("#deadLineSrchType2").append('<option value="reporter">'+ reporter +'</option>');
...
$("input[name=deadLineView_srch1]").change(function(){
    ...
    if(selectedValue === "01") {
	    ...
	    $("#deadLine_div6 p").html(
	    	'<i class="fa-duotone fa-feather text-primary-900"></i>&nbsp;' +
	    	expCnt2 + ': <span id="deadLineViewCnt">0</span>'
	    );
        
        
        $("#deadLineSrchType1 option").remove();
		$("#deadLineSrchType1").append('<option value="">'+ select +'</option>');
		$("#deadLineSrchType1").append('<option value="exp_firm">'+ shipper +'</option>');
		$("#deadLineSrchType1").append('<option value="nation">'+ expCountry +'</option>');
		$("#deadLineSrchType1").append('<option value="reporter">'+ reporter +'</option>');
		
		$("#deadLineSrchType2 option").remove();
		$("#deadLineSrchType2").append('<option value="">'+ select +'</option>');
		$("#deadLineSrchType2").append('<option value="exp_firm">'+ shipper +'</option>');
		$("#deadLineSrchType2").append('<option value="nation">'+ expCountry +'</option>');
		$("#deadLineSrchType2").append('<option value="reporter">'+ reporter +'</option>');
...
*/		{data : 'plantCd', className : "htCenter", wordWrap: false,width: 110,  readOnly:true},  //공장코드
		...
		//{data : 'banIpDay', className : "htCenter", wordWrap: false, width: 90, readOnly:true}, //반입일자
		//{data : 'date4', className : "htCenter", wordWrap: false, width: 90, readOnly:true}, //수입신고기한
		{data : 'rptNo', className : "htCenter", wordWrap: false, width: lang === 'en' ? 180 : 150, readOnly:true},   //신고번호
		{data : 'rptDay', className : "htCenter", wordWrap: false, width: lang === 'en' ? 150 : 90, readOnly:true}, //신고일자
		{data : 'lisDay', className : "htCenter", wordWrap: false, width: lang === 'en' ? 150 : 90,  readOnly:true}, //수리일자
		...
		{data : 'tgPlnMark', className : "htCenter", wordWrap: false,width: lang === 'en' ? 210 : 110, readOnly:true},  //통관계획 부호
		{data : 'tgPlnCot', className : "htCenter", wordWrap: false,width: lang === 'en' ? 180 : 100, readOnly:true},  //통관계획 설명
		/*{data : 'date7', className : "htCenter", wordWrap: false, width: 90,readOnly:true}, //가격확정예정시기
...
function fn_deadLineViewTableHeader(){
	...
	this.deadLineViewHeader = (tableType == "01" && checkBox == "01") ? [
		nabFirm, blNo, entryDt, impDeclareDeadLine, rptNo, rptday, lisDay, releaseDeadLine, paymentDeadLine, paymentDt,
		paymentType, ftaPostDeadLine, reExpPerfDt, revisionDeadLine, entryCharMark, entryPlan, factoryCd, po, reporter
		/*"납세의무자", "B/L","반입일자","수입신고기한","신고번호","신고일자","수리일자","반출기한","납부기한","납부일자",
		 * "납부형태","FTA사후적용기한","재수출이행일자","보정기한","가격확정예정시기", "통관계획부호","통관계획","부서코드","PO","신고인"*/
	] : (tableType == "01" && checkBox == undefined) ? [
		nabFirm, blNo, rptNo, rptday, lisDay, releaseDeadLine, paymentDeadLine, paymentDt,
		paymentType, ftaPostDeadLine, reExpPerfDt, revisionDeadLine, entryCharMark, entryPlan, factoryCd, po, reporter
		/*"납세의무자", "B/L","신고번호","신고일자","수리일자","반출기한","납부기한","납부일자",
		"납부형태","FTA사후적용기한","재수출이행일자","보정기한", "통관계획부호","통관계획","부서코드","PO","신고인"*/
	] :[
		shipper, rptNo, expCountry, rptday, lisDay, shipmentDeadLine, onBoardDt, factoryCd, so, invoice, reporter
		/*"수출자", "신고번호","수출국", "신고일자","수리일자","선적기한","선적일자","부서코드","SO","Invoice","신고인"*/
	];
}
...
function fn_deadLineExcelDownload(){
    ...
    var searchType = $("input:checkbox[name=deadLineView_srch13]:checked").val();
	
    if(searchType == "01"){
    	if(!confirm(msgImpDeclareExcelDown + "\n" + msgImpDeclareExcelDown2)) return;
    	/*if(!confirm("수입신고기한 표시 후 엑셀 다운로드 시 많은 시간이 소요됩니다.\n진행하시겠습니까?")) return;*/
    }
	...
	let deadLineViewCol = new fn_deadLineViewTableCol();
	let deadLineViewHeader = new fn_deadLineViewTableHeader();
	
    exColArr.push(fn_getExcelCol(deadLineViewCol.deadLineViewCol));
	exTitArr.push(fn_getExcelHead(deadLineViewHeader.deadLineViewHeader));
 	exCol = exColArr.join("|||");
	exTit = exTitArr.join("||||");
	exTitDiv = "1|기한관리";
	...
	parameters.srch40 = "기한관리";
	
	fn_loading(true);

	$.ajax({
		 url: "/deadLine/downloadExcel.do",
		 data: parameters,
		 type: 'POST',
		 cache: false,
		 timeout: 200000,
