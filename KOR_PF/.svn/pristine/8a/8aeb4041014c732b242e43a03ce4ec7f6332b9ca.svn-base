...
// Modal upload Action
/*uploaderButtonDocu = document.querySelector('.upload-button-docu');

uploaderButtonDocu.addEventListener('click', () => {
	
	var searchTp = $("input:radio[name=documentView_srch1]:checked").val();
	console.log(searchTp);
	
	var cnt1 = 0 ;
	var checkedData = documentViewHot.getSourceData();
	docuLoadedFiles = [];
	
	for(var i=0; i<checkedData.length; i++){
		if(checkedData[i]["checkBox"] == "yes"){
			
			var rptNo = checkedData[i]["rptNo"];
			var blno = checkedData[i]["blno"];
			var invoiceNo = checkedData[i]["mgCode"];
            
            docuRptNo = rptNo;
            docuBlNo = blno;
            docuInvoiceNo = invoiceNo;
			cnt1 ++;
		}
	}
	
	if (cnt1==0) {alert('업로드할 신고번호를 선택해 주세요.'); return ;}
	if (cnt1>1) {alert('업로드는 한 번에 한 개만 가능합니다.'); return ;}
	...
	modalDocu.classList.remove('hidden')
	modalDocu.classList.add('flex')
	uploaderModalDocu.classList.remove('hidden')
	uploaderModalDocu.classList.add('block')
	
	const fileList = document.querySelector('.file-list-docu');
	fileList.innerHTML = '';
	
	var xData = {};
	xData["srch1"] = docuRptNo;
	xData["srch2"] = docuBlNo;
	xData["srch3"] = docuInvoiceNo;
		...
		$.ajax({
	        ...
	        success : function(data) {
	    		for (var i = 0; i < data.resultList.length; i++) {
					 var docuType = data.resultList[i].docuType
	        	     ...
	        	     li.innerHTML = `
	        	      ...
	        	      <a href="#" target="_blank" class="font-semibold text-xl underline" maxlength='10' style="text-overflow: ellipsis;">
	        	      ` + data.resultList[i].docuFile +`
	        	      </a>
	        	      <span class="text-base pl-3 text-gray-400">
	        	      `+ data.resultList[i].uploadDt +`
	        	      </span>
	        	      <button type="button" class="delete-button hover:opacity-50 text-rose-600" onclick="dropImpFile(event)"><i class="fa-regular fa-trash-can"></i></button>
	        	       </p>
	        	     <select class="min-w-40 text-primary-700 bg-primary-50 border border-primary-500 hover:bg-primary-200 focus:ring-4 focus:outline-none focus:ring-primary-400 font-medium rounded-lg text-sm px-3 py-1.5 text-center inline-flex items-center justify-between dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800" >
        		        <option value="dc" ${docuType === 'DC' ? 'selected' : ''}>신고필증</option>
			            <option value="ci" ${docuType === 'CI' ? 'selected' : ''}>INVOICE</option>
			            <option value="pl" ${docuType === 'PL' ? 'selected' : ''}>PACKING LIST</option>
			            <option value="bl" ${docuType === 'BL' ? 'selected' : ''}>BL</option>
			            <option value="co" ${docuType === 'CO' ? 'selected' : ''}>원산지증명서</option>
			            <option value="rq" ${docuType === 'RQ' ? 'selected' : ''}>요건 서류</option>
			            <option value="cb" ${docuType === 'CB' ? 'selected' : ''}>통합</option>
			            <option value="ot" ${docuType === 'OT' ? 'selected' : ''}>기타</option>
			            <option value="ac" ${docuType === 'AC' ? 'selected' : ''}>정산서</option>
				      </select>
	        	      `;
	        ...
	        success : function(data) {
	    		...
	    		for (var i = 0; i < data.resultList.length; i++) {
	        	     ...
	        	     li.innerHTML = `
	        	       ...
	        	       <button type="button" class="delete-button hover:opacity-50 text-rose-600" onclick="dropExpFile(event)"><i class="fa-regular fa-trash-can"></i></button>
	        	       </p>
	        	       <select class="min-w-40 text-primary-700 bg-primary-50 border border-primary-500 hover:bg-primary-200 focus:ring-4 focus:outline-none focus:ring-primary-400 font-medium rounded-lg text-sm px-3 py-1.5 text-center inline-flex items-center justify-between dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800" >
        		        <option value="dc" ${docuType === 'DC' ? 'selected' : ''}>신고필증</option>
			            <option value="ci" ${docuType === 'CI' ? 'selected' : ''}>INVOICE</option>
...
function dropImpFile(event) {
	const fileElement = event.target.closest('li');
	const fileNameElement = fileElement.querySelector('a');
    const fileName = fileNameElement.innerText.trim();
	sData = {};
	sData["srch1"] = docuRptNo;
	sData["srch2"] = docuBlNo;
	sData["srch3"] = fileName;
	if (confirm("기존 파일을 삭제하시겠습니까?")) {
		
		$.ajax({
			type : "POST",
			url : "/document/dropDocuImpModalFile.do",
			data : sData,
			beforeSend : function(xmlHttpRequest){
				xmlHttpRequest.setRequestHeader("AJAX", "true");
			},
...
function dropExpFile(event) {
	const fileElement = event.target.closest('li');
	const fileNameElement = fileElement.querySelector('a');
	const fileName = fileNameElement.innerText.trim();
	sData = {};
	sData["srch1"] = docuRptNo;
	sData["srch2"] = docuInvoiceNo;
	sData["srch3"] = fileName;
	if (confirm("기존 파일을 삭제하시겠습니까?")) {
		
		$.ajax({
			type : "POST",
			url : "/document/dropDocuExpModalFile.do",
			data : sData,
			beforeSend : function(xmlHttpRequest){
				xmlHttpRequest.setRequestHeader("AJAX", "true");
			},
    ...
    if (isAllChecked == true) {
        if (confirm("전체 파일을 삭제하시겠습니까?")) {
            ...
            for (var i = 0; i < rowData.length; i++) {
                sData["srch2"] = rowData[i].blno;
                sData["srch6"] = rowData[i].rptNo;
            }

            $.ajax({
                type: "POST",
                url: "/document/docuPopupDeleteList.do",
                data: sData,
                beforeSend: function(xmlHttpRequest) {
                    xmlHttpRequest.setRequestHeader("AJAX", "true");
                },
                dataType: "json",
                success: function(data) {
                    docuListHot.alter('remove_row', 0, docuListHot.countRows());
        ...
        if (confirm("파일을 삭제하시겠습니까?")) {
            ...
            for (let i = 0; i < rowData.length; i++) {
                if (rowData[i].checkBox === "yes") {
                    sData["srch2"] = rowData[i].blno;
                    sData["srch3"] = rowData[i].docuFile;
                    sData["srch4"] = rowData[i].uploadDt;
                    sData["srch6"] = rowData[i].rptNo;
