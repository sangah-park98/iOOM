...
<main class="p-2 grow flex flex-col gap-2">
  <form class="w-full shrink-0 grid grid-rows-3 grid-cols-[auto_repeat(5,1fr)_auto_repeat(5,1fr)]
    ...
    <div class="flex flex-row flex-wrap gap-4 col-start-2 col-span-5 row-start-1 items-center *:flex *:flex-row *:items-center *:gap-2 font-medium text-gray-900">
      ...
      <label for="default-radio-5">
        <input
          ...
          class="w-4 h-4 text-primary-600 bg-gray-100 border-gray-300 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
        >
        <span>${customsBefore}</span> <!-- 통관 진행 전 -->
      </label>
    </div>
    <!-- 검색기간 -->
    ...
    <input type="text" id="exportIn_srch4" onkeyup="enterkey()"
    ...
    <input type="text" id="exportIn_srch5" onkeyup="enterkey()"
  ...
  <div class="flex items-center justify-between w-full shrink-0">
    <div class="flex items-center gap-4" id="exportIn_div6">
    ...
    <div class="flex items-center gap-2 px-2">
      ...
      <a id="exportMakeIn2" onclick="fn_openInovice2()" href="#" class="invoice-button text-white bg-primary-700 hover:bg-secondary-800 focus:ring-4 focus:ring-secondary-300 font-medium rounded px-2.5 py-1.5 focus:outline-noneduration-300 row-start-1 text-base">
        <i class="fa-regular fa-feather"></i>
...
<div class="modal-popup fixed top-0 left-0 h-full w-full z-[200] bg-black/50 items-center justify-center duration-300 hidden">
  ...
  <div class="uploader-modal hidden modal-content bg-white shadow-xl relative rounded min-w-96 overflow-hidden">
    <div class="pl-7 pr-5 py-4 text-white bg-primary-900 flex items-center justify-between">
      <h2 class="font-bold text-xl">${docUpload}</h2>
...
<script>
	...
	var customsIng = "${customsIng}";
	var customsBefore = "${customsBefore}";
	var makeCi = "${makeCi}";
	...
	var msgCiFileType = "${msgCiFileType}";
	var msgCiFileOne = "${msgCiFileOne}";
	var msgFileStartToCi = "${msgFileStartToCi}";
	var msgFileInputCiNo = "${msgFileInputCiNo}";
	var msgFileAlreadyCiDone = "${msgFileAlreadyCiDone}";
	var msgFileSave = "${msgFileSave}";
	var msgSelectFile = "${msgSelectFile}";
	var msgNoSelectedFile = "${msgNoSelectedFile}";
	var msgDelSelectedFile = "${msgDelSelectedFile}";
	var msgFileDel = "${msgFileDel}";
	var msgSearchError = "${msgSearchError}";
	var msgUploadFile = "${msgUploadFile}";
</script>
</body>
</html>