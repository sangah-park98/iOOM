...
var kpiImportViewCol = [
	...
	{data : 'impRptDay', className : "htCenter", width: lang === 'en' ? 140 : 120, wordWrap: false, readOnly:true},
	{data : 'rptDay', className : "htCenter", width: 120, wordWrap: false, readOnly:true},
...
var kpiImportViewHead = [
	nabFirm, blNo, rptNo, applicationDt, approvalDt, correctionType,
	failReason, correctionReason, TaxVariation, taxPrice, additonalDuties, customs, lisDay, factoryCd, po
	/*"납세의무자", "B/L", "신고번호", "신청일자", "승인일자", "정정구분", 
	"귀책사유", "정정사유", "세액증감내역", "납부금액", "가산세", "세관", "수리일자", "부서코드", "PO"*/
];
...
var kpiLeadtimeViewCol = [
	...
	{data : 'diff7', className : "htCenter", width: 120, wordWrap: false, readOnly:true},
...
var kpiLeadtimeViewHead = [
	nabFirm, blNo, rptNo, rptday, lisDay, sortingResult, entryPlan, collectionType, inputDay,
	carryInDay, rptDate, lisDate, impExpDt, inAndOut, carryInDeclartion, declartionAccept, acceptRelease, factoryCd, po 
	/*"납세의무자", "B/L", "신고번호", "신고일자", "수리일자", "선별결과", "통관계획", "징수형태", "입항일시", 
	"반입일시", "신고일시", "수리일시", "반출일시", "입항-반입","반입-신고", "신고-수리", "수리-반출", "부서코드", "PO"*/
];
...
//	  });
	...
	$("#kpiViewSrchType1").append('<option value="">'+ select +'</option>'); // 조건선택
	$("#kpiViewSrchType1").append('<option value="nabFirm">'+ nabFirm +'</option>'); // 납세의무자
	$("#kpiViewSrchType1").append('<option value="rptdivi1cont">'+ correctionType +'</option>'); // 정정구분
	$("#kpiViewSrchType1").append('<option value="appDiviCont">'+ failReason +'</option>'); // 귀책사유
	$("#kpiViewSrchType1").append('<option value="reaseonDoc">'+ correctionReason +'</option>'); // 정정사유
	$("#kpiViewSrchType1").append('<option value="cusNm">'+ customs +'</option>'); // 세관
	$("#kpiViewSrchType1").append('<option value="plant">'+ factoryCd +'</option>'); // 부서코드
	$("#kpiViewSrchType1").append('<option value="po">'+ po +'</option>'); // PO
	
	$("#kpiViewSrchType2 option").remove();
	$("#kpiViewSrchType2").append('<option value="">'+ select +'</option>');
	$("#kpiViewSrchType2").append('<option value="nabFirm">'+ nabFirm +'</option>');
	$("#kpiViewSrchType2").append('<option value="rptdivi1cont">'+ correctionType +'</option>');
	$("#kpiViewSrchType2").append('<option value="appDiviCont">'+ failReason +'</option>');
	$("#kpiViewSrchType2").append('<option value="reaseonDoc">'+ correctionReason +'</option>');
	$("#kpiViewSrchType2").append('<option value="cusNm">'+ customs +'</option>');
	$("#kpiViewSrchType2").append('<option value="plant">'+ factoryCd +'</option>');
	$("#kpiViewSrchType2").append('<option value="po">'+ po +'</option>');
...
// 검색구분 변경
//검색구분 변경
$("input[name=kpiView_srch1]").change(function(){
...
function fn_kpiShowSearchOption(option) {
	if("import" == option) {
		...
		$("#kpiViewSrchType1 option").remove();
		$("#kpiViewSrchType1").append('<option value="">'+ select +'</option>');
		$("#kpiViewSrchType1").append('<option value="expFirm">'+ shipper +'</option>'); // 수출자
		$("#kpiViewSrchType1").append('<option value="sendDiviCont">'+ failReason +'</option>'); // 귀책사유
		$("#kpiViewSrchType1").append('<option value="modiCot1">'+ correctionReason +'</option>'); // 정정사유
		$("#kpiViewSrchType1").append('<option value="rece">'+ receiveStatus +'</option>'); // 수신상태
		$("#kpiViewSrchType1").append('<option value="rptCusCont">'+ customs +'</option>'); // 세관
		$("#kpiViewSrchType1").append('<option value="plant">'+ factoryCd +'</option>'); // 부서코드
		$("#kpiViewSrchType1").append('<option value="so">'+ so +'</option>'); // SO
		
		$("#kpiViewSrchType2 option").remove();
		$("#kpiViewSrchType2").append('<option value="">'+ select +'</option>');
		$("#kpiViewSrchType2").append('<option value="expFirm">'+ shipper +'</option>');
		$("#kpiViewSrchType2").append('<option value="sendDiviCont">'+ failReason +'</option>');
		$("#kpiViewSrchType2").append('<option value="modiCot1">'+ correctionReason +'</option>');
		$("#kpiViewSrchType2").append('<option value="rece">'+ receiveStatus +'</option>');
		$("#kpiViewSrchType2").append('<option value="rptCusCont">'+ customs +'</option>');
		$("#kpiViewSrchType2").append('<option value="plant">'+ factoryCd +'</option>');
		$("#kpiViewSrchType2").append('<option value="so">'+ so +'</option>');
		...
		$("#kpiViewSrchType1 option").remove();
		$("#kpiViewSrchType1").append('<option value="">'+ select +'</option>');
		$("#kpiViewSrchType1").append('<option value="nabFirm">'+ nabFirm +'</option>'); // 납세자상호
		$("#kpiViewSrchType1").append('<option value="receResult">'+ sortingResult +'</option>'); // 선별결과
		$("#kpiViewSrchType1").append('<option value="tgCont">'+ entryPlan +'</option>'); // 통관계획
		$("#kpiViewSrchType1").append('<option value="levForm">'+ collectionType +'</option>'); // 징수형태
		$("#kpiViewSrchType1").append('<option value="plant">'+ factoryCd +'</option>'); // 부서코드
		$("#kpiViewSrchType1").append('<option value="po">'+ po +'</option>'); // PO
		...
		$("#kpiViewSrchType2").append('<option value="">'+ select +'</option>');
		$("#kpiViewSrchType2").append('<option value="nabFirm">'+ nabFirm +'</option>');
		$("#kpiViewSrchType2").append('<option value="receResult">'+ sortingResult +'</option>');
		$("#kpiViewSrchType2").append('<option value="tgCont">'+ entryPlan +'</option>');
		$("#kpiViewSrchType2").append('<option value="levForm">'+ collectionType +'</option>');
		$("#kpiViewSrchType2").append('<option value="plant">'+ factoryCd +'</option>');
		$("#kpiViewSrchType2").append('<option value="po">'+ po +'</option>');
	}
...
function fn_kpiGetSearchForm(){
	...
	sData["srchType1"] = $("#kpiViewSrchType1 option:selected").val();
	sData["srchText1"] = $("#kpiViewSrchText1").val();
	
	sData["srchType2"] = $("#kpiViewSrchType2 option:selected").val();
	sData["srchText2"] = $("#kpiViewSrchText2").val();
...
function fn_kpiClearSerachOption(){
	...
	$("#kpiViewSrchType1").val("");
	$("#kpiViewSrchType2").val("");
	
	$("#kpiViewSrchText1").val("");
	$("#kpiViewSrchText2").val("");
...
function fn_searchImport(callback) {
	...
	$.ajax({
	    ...
	    success : function(data) {
	    	...
	    	$("#kpiImportViewCnt").html(fn_setComma(totCnt));
...
function fn_searchExport() {
	...
	$.ajax({
	    ...
	    success : function(data) {
	    	...
	    	var totCnt = (data.resultList.length > 0) ? data.resultList[0].cnt : 0;
	    	$("#kpiExportViewCnt").html(fn_setComma(totCnt));
...
function fn_searchLeadtime() {
	...
	$.ajax({
	    ...
	    success : function(data) {
	    	...
	    	var totCnt = (data.resultList.length > 0) ? data.resultList[0].cnt : 0;
	    	$("#leadtimeViewCnt").html(fn_setComma(totCnt));
...
function fn_setSummary(table, col, scroll, index) {
    ...
    var tempObj = {"summary": true}
    	tempObj[col[0]["data"]] = total; // total -> 합계
