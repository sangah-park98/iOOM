...
function fn_expUpdateTableCol(){
	...
	this.expUpdateViewCol =  [
		...
		{data : 'modiSeq', className : "htCenter", width: lang === 'en' ? 100 : 60, wordWrap: false, className : "htCenter", readOnly:true},
...
function fn_expUpdateTableHeader(){
	...
	this.expUpdateViewHeader = [
		"", status, shipper, invoiceNo, rptNo, correctionSeq, applicationDt, 
		approvalDt, correctionReason, customs, lisDay, factoryCd, so
		/*"", "상태", "수출자상호", "Invoice번호", "신고번호", "정정차수", "신청일자", 
		 * "승인일자", "정정사유", "세관", "수리일자", "부서코드", "SO"*/
	 ] ;
}
...
function fn_expDtlUpdViewTableHeader(){
	this.expDtlUpdViewHeader = [
		rptNo, lanNo, specNo, correctionName, correctionBefore, correctionAfter, approvalNo, serialNo, containerSerialNo
