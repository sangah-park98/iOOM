...
var tradeGapLanHot;
var tradeGapLanSettings;
var tradeGapIndex = 9999;
var tradeGapScrollTp = true;
var tradeGapLanIndex = 9999;
var tradeGapLanScrollTp = true;
...
$( document ).ready(function() {
	  ...
	  var tradeGapLanElement = document.querySelector('#tradeGapLanTable');
	  var tradeGapLanElementContainer = tradeGapLanElement.parentNode;
	  tradeGapLanHot = new Handsontable(tradeGapLanElement, tradeGapLanSettings);

	  fn_changeTradeGap('read');
	 
	  //scroll 이벤트
	  fn_tradeGapasEventReg();
	  fn_tradeGapLanasEventReg();
...
// 스크롤할 때 행이 자동으로 추가 로드될 수 있게 하는 함수
function fn_tradeGapLanasEventReg(){
	
	$("#tradeGapTable .wtHolder").scroll(function(){
		//fn_tradeGapFileList(row, col)
		var scrollTop = $("#tradeGapLanTable .wtHolder").scrollTop();
		var countPerPage = $("#tradeGapPageCnt option:selected").val();
		var rowHeight = tradeGapLanHot.getRowHeight();
		...
		if(tradeGapLanScrollTp && tradeGapLanIndex != 9999 && scrollTop >= (countPerPage * tradeGapLanIndex * rowHeight) + addCnt){
			fn_tradeGapLanScroll();
		}
	});
}
...
function fn_tradeGapLanScroll(){
	...
	$.ajax({
		type : "POST",
		url : "/trade/selectTradeGapLanList.do",
		data : JSON.stringify(data),
		contentType: "application/json; charset=utf-8",
		beforeSend : function(xmlHttpRequest){
			xmlHttpRequest.setRequestHeader("AJAX", "true");
		...
		dataType: "json",
		success : function(data) {
			var getData = tradeGapLanHot.getSourceData();
			var meargeJson = getData.concat(data.resultList);
			tradeGapLanHot.loadData(meargeJson);
			tradeGapLanScrollTp = true;
			fn_loading(false);
		},
		error : function(e, textStatus, errorThrown) {
			if(e.status == 400){
...
function fn_searchTradeGapLan(){
	...
	$.ajax({
		...
		dataType: "json",
		success : function(data) {
			tradeGapLanHot.loadData([]);
			tradeGapLanHot.loadData(data.resultList);
			var totCnt = (data.resultList.length > 0) ? data.resultList[0].cnt : 0;
			$("#tradeGapLanPageCnt").text(totCnt); 
			fn_loading(false);
		},
		error : function(e, textStatus, errorThrown) {
			if(e.status == 400){
...
// 테이블 컬럼
function fn_tradeGapLanTableCol(){
	// 사용여부
	var tradeType = $("input:radio[name=tradeGap_srch1]:checked").val(); 
	
	this.tradeGapLanCol = [
		{data : 'tradeType', className : "htCenter", width: 120, wordWrap: false, readOnly:true},
		{data : 'regDt', className : "htCenter", width: 120, wordWrap: false, readOnly:true},
		{data : 'tradeNo', className : "htCenter", width: 200, wordWrap: false, readOnly:true},
		{data : 'tradeItemCd', className : "htCenter", width: 120, wordWrap: false, readOnly:true},
		{data : 'tradeQty', className : "htRight", width: 90, wordWrap: false, readOnly:true},
		{data : 'tradeRemark', className : "htCenter", width: 130, wordWrap: false, readOnly:true},
		{data : 'tradeItemNote', className : "htCenter", width: 130, wordWrap: false, readOnly:true},
		] ;
}
...
// 테이블 헤더
function fn_tradeGapLanTableHeader(){
	
	
	this.tradeGapLanHeader = [
		"보정 유형", "보정 일자", "보정 신고/입고 번호", "보정 품목 코드", "보정 수량" , "보정 사유" , "보정 품목 비고"
		] ;
}
...
// 테이블 히든컬럼
function fn_tradeGapLanTableHidden(){
	var tableType = $("input:radio[name=tradeGapType]:checked").val();
	this.tradeGapLanHidden = [];
}

// table
function fn_handsonGridTradeGapOption(col, header, hidden){
	var tableType = $("input:radio[name=tradeGapType]:checked").val();
...
// table
function fn_handsonGridTradeGapLanOption(col, header, hidden){
	var tableType = $("input:radio[name=tradeGapType]:checked").val();
...
function fn_changeTradeGapType(type){
	...
	let tradeGapLanCol = new fn_tradeGapLanTableCol();
	let tradeGapLanHeader = new fn_tradeGapLanTableHeader();
	let tradeGapLanHidden = new fn_tradeGapLanTableHidden();
	...
	col2 = tradeGapLanCol.tradeGapLanCol;
	header2 = tradeGapLanHeader.tradeGapLanHeader;
	hidden2 = tradeGapLanHidden.tradeGapLanHidden;
	
	tradeGapHot.updateSettings(fn_handsonGridTradeGapOption(col, header, hidden));
	tradeGapLanHot.updateSettings(fn_handsonGridTradeGapLanOption(col2, header2, hidden2));
	
	fn_searchTradeGap();
	fn_searchTradeGapLan();
};
